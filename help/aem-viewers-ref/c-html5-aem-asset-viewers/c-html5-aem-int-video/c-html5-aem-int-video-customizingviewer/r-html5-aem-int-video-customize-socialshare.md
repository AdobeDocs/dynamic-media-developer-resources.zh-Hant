---
title: 社會份額
description: 預設情況下，社交共用工具顯示在右上角。 它由一個按鈕和一個面板組成，當用戶按一下或點擊某個按鈕時，面板會展開，並包含各個共用工具。
...
預設情況下，社交共用工具顯示在右上角。 它由一個按鈕和一個面板組成，當用戶按一下或點擊某個按鈕時，面板會展開，並包含各個共用工具。
...
設定社交共用工具，該工具從頂部放置四個像素，從查看器容器右側放置五個像素，大小為28 x 28像素。
...
設定社交共用工具按鈕，該按鈕顯示四個不同按鈕狀態中每個狀態的不同影像。
