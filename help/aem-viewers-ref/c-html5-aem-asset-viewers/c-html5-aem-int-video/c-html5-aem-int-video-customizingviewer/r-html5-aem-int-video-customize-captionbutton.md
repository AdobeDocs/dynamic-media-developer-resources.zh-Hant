---
title: 標題按鈕
description: 此按鈕可切換關閉的字幕顯示。 如果未指定標題參數，則不可見。
...
此按鈕可切換關閉的字幕顯示。 如果未指定標題參數，則不可見。

<!--<a id="section_061E550C1C1D4DB2BD663A898895B38C"></a>-->

您可以使用CSS，相對於包含此按鈕的控制欄來調整、外觀和定位此按鈕。
...
>[!NOTE]
>
>此按鈕支援 `state` 和 `selected` 屬性選擇器，可用於將不同的外觀應用於不同的按鈕狀態。 特別是， `selected='true'` 對應於字幕可見和 `selected='false'` 在隱藏字幕時使用。
...
設定28 x 28像素的隱藏字幕按鈕。 按鈕必須從頂部放置四個像素，從控制欄的右邊緣放置68個像素。 而且，當選定或未選定時，它必須針對四個不同按鈕狀態中的每個狀態顯示不同的影像。
