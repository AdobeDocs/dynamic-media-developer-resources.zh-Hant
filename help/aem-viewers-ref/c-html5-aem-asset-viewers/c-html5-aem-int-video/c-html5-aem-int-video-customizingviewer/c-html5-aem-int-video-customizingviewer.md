...
檢視器隨附兩個現成可用的CSS檔案，分別適用於「淺色」和「深色」色彩配置。 預設會使用「深色」版本，但使用以下標準CSS很容易切換至「淺色」版本：
...
建立自訂CSS時，請記得檢視器會將`.s7interactivevideoviewer`類別指派給其容器DOM元素。 如果您使用透過`style=`命令傳遞的外部CSS檔案，請在CSS規則的子系選取器中使用`.s7interactivevideoviewer`類別作為父類別。 如果您正在網頁上執行內嵌樣式，請使用容器DOM元素的ID來限定此選取器，如下所示：

`#<containerId>.s7interactivevideoviewer`

## 建立回應式設計的CSS {#section-0bb49aca42d242d9b01879d5ba59d33b}

您可以在CSS中鎖定不同的裝置和內嵌大小，讓內容顯示方式因使用者裝置或特定網頁版面配置而異。 此方法包含但不限於不同的版面、使用者介面元素大小和圖稿解析度。

檢視器支援兩種建立回應式設計CSS的機制： CSS標籤和標準CSS媒體查詢。 您可以單獨或一起使用這些機制。

**CSS標籤**

為協助建立回應式設計CSS，檢視器支援CSS標籤。 這些標籤是特殊的CSS類別。 它們會根據執行階段檢視器大小和目前裝置使用的輸入型別，動態指派給頂層檢視器容器元素。

第一組CSS標籤包括`.s7size_large`、`.s7size_medium`和`.s7size_small`類別。 它們會根據檢視器容器的執行時間區域套用。 如果檢視器區域等於或大於一般桌上型熒幕的大小，則使用`.s7size_large`；如果區域接近一般平板電腦裝置，則指派`.s7size_medium`。 對於類似行動電話熒幕的區域，已設定`.s7size_small`。 這些CSS標籤的主要用途是為不同的熒幕和檢視器大小建立不同的使用者介面配置。

第二組CSS標籤包含`.s7mouseinput`和`.s7touchinput`。 如果目前裝置具有觸控輸入功能，則會設定標籤`.s7touchinput`；否則會使用`.s7mouseinput`。 這些標籤主要用於為不同的輸入型別建立具有不同熒幕大小的使用者介面輸入元素，因為通常觸控輸入需要更大的元素。

第三組CSS標籤包含`.s7device_landscape`和`.s7device_portrait`。 如果觸控裝置為橫向方向，則會設定標籤`.s7device_landscape`；當觸控裝置旋轉為縱向方向時，會使用`.s7device_portrait`。 這些CSS標籤僅供案頭系統使用。

下列範例CSS在含有滑鼠輸入的系統上將播放/暫停按鈕大小設定為28x28畫素，在觸控裝置上將設定為56x56畫素。 此外，如果檢視器大小大幅縮減，則會完全隱藏按鈕：
...
在下個範例中，如果觸控裝置為直向方向，視訊控制列會位於檢視器底部上方138畫素的位置。 在所有其他情況下，該區段會移至檢視器的底部：
...
CSS sprite是一種不同的方法，將所有元素狀態的影像圖稿合併到稱為「sprite」的單一PNG檔案中。 此類「sprite」具有特定元素的所有視覺狀態，且會逐一定位。 使用拼寫來設定使用者介面元素的樣式時，CSS中所有不同的狀態都會參照相同的sprite影像。 此外，`background-position`屬性會用於每個狀態，以指定使用「sprite」影像的哪一部分。 您可以以任何適當的方式建構「Sprite」影像。 檢視器通常會垂直棧疊。 以下是以「sprite」為基礎的範例，說明如何稍早設定相同全熒幕按鈕的樣式：
