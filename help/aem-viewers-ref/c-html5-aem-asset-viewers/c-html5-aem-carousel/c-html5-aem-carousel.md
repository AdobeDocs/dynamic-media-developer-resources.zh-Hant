...
建議您使用現成可用的HTML頁面來進行快顯作業模式。 在這種情況下，它會被呼叫， `CarouselViewer.html` 並位於標準IS- `html5/` Viewer部署的子資料夾中：
...
1. 將檢視器JavaScript檔案新增至您的網頁。
   ...
   建立檢視器時，您必須在HTML標題中新增指令碼標籤。 在您使用檢視器API之前，請確定您已加入 [!DNL CarouselViewer.js]。 此檔 [!DNL CarouselViewer.js] 案位於標準IS- [!DNL html5/js/] Viewer部署的子資料夾下：
...
1. 設定檢視器大小
   ...
   您可以設定檢視器的靜態大小，方法是以絕對單位 `.s7carouselviewer` 來宣告頂層CSS類別，或使用修飾 `stagesize` 元。

   您可以直接將大小調整置於CSS中的HTML頁面，或自訂檢視器CSS檔案中，這些檔案稍後會指派給AEM Assets中的檢視器預設記錄——隨選，或使用命令明確傳遞 `style` 。

   如需使 [用CSS設定檢視器樣式的詳細資訊](../../c-html5-aem-asset-viewers/c-html5-aem-carousel/c-html5-aem-carousel-customizingviewer/c-html5-aem-carousel-customizingviewer.md#concept-73a8546acdb444a387c49969ceca57d0) ，請參閱自訂轉盤檢視器。
...
1. 建立和初始化檢視器。
   ...
   完成上述步驟後，您將建立類的實例、將所有配置資訊傳遞給其建構子 `s7viewers.CarouselViewer` ，並在查看器實例 `init()` 上調用方法。 設定資訊會以JSON物件的形式傳遞至建構函式。 至少，此物件應具有欄 `containerId` 位，其中包含檢視器容器ID名稱和內嵌 `params` JSON物件，以及檢視器支援的設定參數。 在此情況下，物 `params` 件必須至少將「影像伺服URL」傳遞為屬 `serverUrl` 性，並將初始資產傳遞為 `asset` 參數。 以JSON為基礎的初始化API可讓您使用一行程式碼來建立和啟動檢視器。
   ...
   以下是建立檢視器例項、將最小必要設定選項傳遞至建構函式並呼叫方法的 `init()` 範例。 範例假設 `carouselViewer` 是檢視器例項； `s7viewer` 是佔位符的名稱 `DIV`;是 `https://adobedemo62-h.assetsadobe.com/is/image` 影像伺服URL, `/content/dam/dm-public-facing-live-demo-page/04_shoppable_carousel/05_shoppable_banner` 且是資產：
...
上述所有步驟都與固定大小內嵌相同。 將容器新 `DIV` 增至現有 `"holder"` 的 `DIV`。 以下程式碼為完整範例。 請注意，當重新調整瀏覽器大小時，檢視器大小會如何變更，以及檢視器外觀比例如何符合資產。
