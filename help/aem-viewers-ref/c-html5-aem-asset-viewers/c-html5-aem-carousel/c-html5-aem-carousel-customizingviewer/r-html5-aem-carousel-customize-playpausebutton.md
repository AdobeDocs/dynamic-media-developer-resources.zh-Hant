...
只有在`CarouselViewer.autoplay`參數設為`1`時，按鈕才會顯示；否則會隱藏。 您可以使用CSS來比較此按鈕的大小、外觀和位置（相對於包含該按鈕的控制欄）。
...
例如，假設您要設定播放暫停按鈕，即28 x 28像素。 您希望按鈕從底部放置17個像素，從檢視器左邊放置12個像素。 而且，您希望它在選取或未選取時，針對四個不同按鈕狀態中的每一個顯示不同的影像。
