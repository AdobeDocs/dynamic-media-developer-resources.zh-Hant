...
檢視器提供4個現成可用的CSS檔案，以數值和虛線設定指標顯示，每個都採用「淺色」和「深色」色彩配置。 預設會使用「點狀光」版本，但是使用不同的標準CSS並設定`SetIndicator.mode`參數，就可輕鬆切換至不同的版本。 其他標準CSS位於下列位置：
...
建立自訂CSS時，請記住檢視器會指派`.s7carouselviewer`類別給其容器DOM元素。 如果您使用與`style=`命令一起傳遞的外部CSS檔案，請在CSS規則的子體選擇器中使用`.s7carouselviewer`類作為父類。 如果您要在網頁上新增內嵌樣式，請另外以容器DOM元素的ID來限定此選取器，如下所示：
