---
title: 自定義旋轉軸查看器
description: Carousel查看器的所有可視自定義和大多數行為自定義都通過建立自定義CSS來完成。
keywords: 響應
...
# 自定義旋轉軸查看器{#customizing-carousel-viewer}

Carousel查看器的所有可視自定義和大多數行為自定義都通過建立自定義CSS來完成。
...
為查看器提供了四個現成CSS檔案，用於數字和虛線設定指示器，每個檔案採用「淺」和「暗」顏色方案。 預設情況下使用「點光」版本，但使用不同的標準CSS並設定 `SetIndicator.mode` 的下界。 其他標準CSS位於以下位置：
...
建立自定義CSS時，切記查看器指定 `.s7carouselviewer` 類到其容器DOM元素。 如果使用的是與 `style=` 命令，使用 `.s7carouselviewer` 類作為CSS規則的後代選擇器中的父類。 如果要在網頁上添加嵌入樣式，還應使用容器DOM元素的ID來限定此選擇器，如下所示：
