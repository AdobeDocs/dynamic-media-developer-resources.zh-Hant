...
輪播檢視器的所有視覺化自訂和大部分的行為自訂，都是透過建立自訂CSS來完成。
...
檢視器提供四個現成可用的CSS檔案，分別用於數值和點線設定指標，每個指標都採用「淺」和「深」色彩配置。 預設會使用「虛線」版本，但使用不同標準CSS並設定`SetIndicator.mode`參數，即可輕鬆切換至不同版本。 您可在下列位置找到其他標準CSS:
