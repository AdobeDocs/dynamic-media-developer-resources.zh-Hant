...
檢視器隨附四個現成可用的CSS檔案（用於數值和點狀設定指標），每個都採用「淺色」和「深色」色彩配置。 預設會使用「點狀光」版本，但透過使用不同的標準CSS並設定 `SetIndicator.mode` 引數。 其他標準CSS可在下列位置找到：
...
建立自訂CSS時，請記住檢視器會指派 `.s7carouselviewer` 類別至其容器DOM元素。 如果您使用傳遞的外部CSS檔案， `style=` 命令，使用 `.s7carouselviewer` 類別作為CSS規則之子系選擇器中的父類別。 如果您在網頁上新增內嵌樣式，請一併使用容器DOM元素的ID來限定此選取器，如下所示：
...
為了協助建立回應式設計的CSS，檢視器支援CSS標籤。 這些標籤是特殊的CSS類別，會動態指派給頂層檢視器容器元素。 它們是根據執行階段檢視器大小和目前裝置使用的輸入型別。
...
第二組CSS標籤包含 `.s7mouseinput` 和 `.s7touchinput`. CSS標籤 `.s7touchinput` 若目前裝置為觸控輸入，則設為。 否則， `.s7mouseinput` 已使用。 這些標籤主要用於為不同的輸入型別建立具有不同熒幕大小的使用者介面輸入元素，因為通常觸控輸入需要更大的元素。
...
以下是以「sprite」為基礎的範例，說明如何設定相同連結區圖示的樣式：
