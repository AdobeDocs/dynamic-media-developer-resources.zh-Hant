...
若要使用VR HMD掛載，必須在檢視器的設定中將`Video360Player.vrrender`修飾元設為`1`，以強制進行立體轉譯。
...
1. 設定檢視器大小
   ...
   您可以透過以絕對單位宣告`.s7video360viewer`頂層CSS類別的檢視器，或使用`stagesize`修飾元來設定檢視器的靜態大小。

   您可以將CSS的大小直接放在HTML頁面上，或放在自訂檢視器CSS檔案中(稍後會在Adobe Experience Manager Assets中指派給檢視器預設集記錄 — 隨選)，或使用`style`命令明確傳遞。
   ...
   您可以在AEM Assets的檢視器預設集記錄中設定`stagesize`修飾元 — 隨選。 或者，您可以使用`params`集合的檢視器初始化程式碼來明確傳遞它，或以API呼叫的形式傳遞，如Command Reference區段中所述，如下所示：
...
1. 正在建立和初始化檢視器。
   ...
   完成上述步驟後，您會建立`s7viewers.Video360Viewer`類別的執行個體、將所有組態資訊傳遞至其建構函式，並在檢視器執行個體上呼叫`init()`方法。 組態資訊會以JSON物件的形式傳遞至建構函式。 此物件至少應有`containerId`欄位，其包含檢視器容器ID的名稱，以及含有檢視器支援之組態引數的巢狀`params` JSON物件。
   ...
   * 檢視器執行個體是`video360Viewer`。
   * 預留位置`DIV`的名稱為`s7viewer`。
   * 影像伺服URL為`https://s7d9.scene7.com/is/image`。
   * 視訊伺服器URL是`https://s7d9.scene7.com/is/content`。
...
上述所有步驟與內嵌固定大小相同。 將容器DIV新增至現有的`"holder"` DIV。
