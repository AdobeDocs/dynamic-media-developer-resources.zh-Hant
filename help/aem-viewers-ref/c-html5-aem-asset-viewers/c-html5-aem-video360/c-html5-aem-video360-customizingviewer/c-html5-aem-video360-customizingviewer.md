---
title: 自定義Video360查看器
description: Video360查看器的所有可視自定義和大多數行為自定義都通過建立自定義CSS來完成。
keywords: 響應
...
Video360查看器的所有可視自定義和大多數行為自定義都通過建立自定義CSS來完成。
...
建立自定義CSS時，請記住查看器會指定 `.s7video360viewer` 類到其容器DOM元素。 如果使用與 `style=` 命令，使用 `.s7video360viewer` 類作為CSS規則的後代選擇器中的父類。 如果要在網頁上執行嵌入樣式，還應使用容器DOM元素的ID來限定此選擇器，如下所示：
...
查看器支援建立響應性設計的CSS的兩種機制：CSS標籤和標準CSS媒體查詢。 可以單獨或一起使用標籤或查詢。

**CSS標籤**

為幫助建立響應性設計的CSS，查看器支援CSS標籤。 這些標籤是動態分配給頂級查看器容器元素的特殊CSS類。 它們基於運行時查看器大小和當前設備上使用的輸入類型。

第一組CSS標籤包括 `.s7size_large`。 `.s7size_medium`, `.s7size_small` 類。 它們基於查看器容器的運行時區域應用。 如果查看器區域等於或大於公共案頭顯示器的大小，則 `.s7size_large` 的下界；如果區域接近普通的平板電腦設備 `.s7size_medium` 已分配。 對於類似於手機螢幕的區域， `.s7size_small` 的子菜單。 這些CSS標籤的主要目的是為不同的螢幕和查看器大小建立不同的用戶介面佈局。

第二組CSS標籤包含 `.s7mouseinput` 和 `.s7touchinput`。 標籤 `.s7touchinput` 當當前設備具有觸摸輸入功能時設定；否則， `.s7mouseinput` 的子菜單。 這些標籤主要用於為不同的輸入類型建立具有不同螢幕大小的用戶介面輸入元素，因為通常觸摸輸入需要較大的元素。 如果設備同時具備滑鼠輸入和觸摸功能， `.s7touchinput` 設定，並且查看器呈現一個觸摸友好用戶介面。
...
應用於HTML5移動查看器時，按下列順序使用四個CSS媒體查詢：
...
以下是適用於Video360 Viewer的用戶介面元素參考文檔：
