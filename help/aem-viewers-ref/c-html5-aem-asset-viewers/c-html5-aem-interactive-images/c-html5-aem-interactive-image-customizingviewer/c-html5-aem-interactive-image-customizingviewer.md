...
建立自訂CSS時，請記住檢視器會將類 `.s7interactiveimage` 別指派給其容器DOM元素。 如果您使用隨命令傳遞的外部CSS檔案，請 `style=` 在CSS規 `.s7interactiveimage` 則的子體選擇器中，使用類別做為上層類別。 如果您要在網頁上新增內嵌樣式，請另外以容器DOM元素的ID來限定此選取器，如下所示：
