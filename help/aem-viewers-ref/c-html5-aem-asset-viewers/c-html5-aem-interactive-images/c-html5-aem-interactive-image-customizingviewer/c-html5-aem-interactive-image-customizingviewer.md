---
title: 自訂互動式影像檢視器
description: 互動式影像檢視器的所有視覺化自訂和大部分行為自訂都是透過建立自訂CSS完成的。
keywords: 回應式
...
互動式影像檢視器的所有視覺化自訂和大部分行為自訂都是透過建立自訂CSS完成的。
...
建立自訂CSS時，請記得檢視器會將`.s7interactiveimage`類別指派給其容器DOM元素。 如果您使用透過`style=`命令傳遞的外部CSS檔案，請在CSS規則的子系選取器中使用`.s7interactiveimage`類別作為父類別。 如果您在網頁上新增內嵌樣式，請一併以容器DOM元素的ID限定此選取器，如下所示：
...
第二組CSS標籤包含`.s7mouseinput`和`.s7touchinput`。 如果目前裝置有觸控輸入，則會設定CSS標籤`.s7touchinput`。 否則，會使用`.s7mouseinput`。 這些標籤主要用於為不同的輸入型別建立具有不同熒幕大小的使用者介面輸入元素，因為通常觸控輸入需要更大的元素。
