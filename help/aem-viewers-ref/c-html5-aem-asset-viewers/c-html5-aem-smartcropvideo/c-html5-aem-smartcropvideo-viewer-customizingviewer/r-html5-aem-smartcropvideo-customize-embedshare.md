...
示例 — 要設定28 x 28像素的「嵌入共用」按鈕，並為四個不同按鈕狀態中的每個狀態顯示不同的影像：
...
示例 — 要設定對話框標題，其中包含填充、24 x 14像素表徵圖、粗體16點標題和28 x 28像素「關閉」按鈕。 最後，讓它從頂部放置兩個像素，從對話框容器右側放置兩個像素：
...
對話框標籤工具提示可以本地化。 請參閱 [用戶介面元素的本地化](../../../c-html5-aem-asset-viewers/c-html5-aem-smartcropvideo/r-html5-aem-smartcropvideo-viewer-localization.md#concept-1d5ca2d8480f4064a51eddba13940aad) 的子菜單。
...
嵌入大小標籤和下拉清單位於對話框的底部，並放入由以下CSS類選擇器控制的容器中：
...
在嵌入大小組合框中選擇「自定義大小」選項時，該對話框右側顯示兩個額外的輸入欄位，允許用戶輸入自定義嵌入大小。 這些欄位包裝在由以下CSS類選擇器控制的容器中：
...
示例 — 將自定義大小的輸入欄位設定為具有一個像素的灰度邊框、邊距、填充和70像素寬：
