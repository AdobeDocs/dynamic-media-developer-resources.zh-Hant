...
在播放視頻時，掃描器「旋鈕」也移動，以指示回放期間視頻的當前時間位置。 視頻掃描器總是取控制條的整個寬度。 通過CSS可以對視頻掃描器進行蒙皮、改變其高度和垂直位置。
...
視頻掃描器工具尖端可被本地化。 請參閱 [用戶介面元素的本地化](../../../c-html5-aem-asset-viewers/c-html5-aem-smartcropvideo/r-html5-aem-smartcropvideo-viewer-localization.md#concept-1d5ca2d8480f4064a51eddba13940aad) 的子菜單。

**示例**  — 設定視頻查看器，其視頻掃描器具有10像素高的自定義軌道顏色。 最後，讓它位於控制欄的上邊緣和左邊緣的10個像素和35個像素。
