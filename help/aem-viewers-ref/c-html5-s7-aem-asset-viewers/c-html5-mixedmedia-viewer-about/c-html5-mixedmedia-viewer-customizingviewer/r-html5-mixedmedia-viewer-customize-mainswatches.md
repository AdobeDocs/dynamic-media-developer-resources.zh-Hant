---
description: 主色票由一行縮圖影像組成，左側和右側有可選的捲動按鈕。 只有當所有縮圖都無法放入容器寬度時，捲動按鈕才會顯示在案頭上。 在行動裝置上，或如果縮圖可符合容器寬度，則不會顯示捲動按鈕。
...
覆蓋圖支援`type`屬性選取器，具有下列可能值：`image`（適用於單一影像）、`swatchset`（適用於色票集）、`spinset`（適用於回轉集）和`video`（適用於單一視訊或最適化視訊集）。
