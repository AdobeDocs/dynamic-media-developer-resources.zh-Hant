...
seo-description: 主色票由一列縮圖影像組成，左側和右側有選用的捲動按鈕。 如果所有縮圖都無法符合容器的寬度，捲動按鈕只會顯示在案頭上。 在行動裝置上，或如果縮圖可符合容器寬度，則不會顯示捲動按鈕。
seo-title: 主色票
...
>[!NOTE]
>
>縮圖支援 `state` 屬性選擇器，可用來將不同外觀套用至不同的縮圖狀態。 尤其是， `state="selected"` 與主視圖中當前顯示的影像的縮略圖相對應， `state="default"` 與其餘的縮略圖相對應， `state="over"` 並用於滑鼠懸停。
...
覆蓋支援具 `type` 有下列可能值的屬性選取器： `image` （適用於單一影像）、 `swatchset` （適用於色票集）、 `spinset` （適用於回轉集） `video` 和（適用於單一影片或最適化視訊集）。
...
>[!NOTE]
>
>此按鈕支援屬 `state` 性選擇器，可用來將不同的外觀套用至不同的按鈕狀態： `up`、 `down``over`和 `disabled`。
