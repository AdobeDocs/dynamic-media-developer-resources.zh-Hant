...
seo-description: 自旋指示器覆蓋在自旋視圖區域上。 當影像處於重設狀態時，就會顯示它，而且它也會依循圖示效果參數。
seo-title: 回轉檢視圖示效果
...
# 回轉檢視圖示效果{#spin-view-icon-effect}
...
回轉指示器支 `state` 持屬性選擇器，該屬性選擇器在單維回轉集 `spin_1D` 的情況下設定為，在多維回轉集 `spin_2D` 的情況下設定為。
