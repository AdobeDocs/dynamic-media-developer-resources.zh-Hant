---
title: 播放/暫停按鈕
description: 當用戶選擇視頻內容時，播放/暫停按鈕使視頻播放器播放或暫停視頻內容。
...
當用戶選擇視頻內容時，「播放/暫停」按鈕會使視頻播放器播放或暫停視頻內容。
...
>
>`replay='true'` 當視頻到達結束時，選擇按鈕將從開始重新開始播放時，將設定此屬性。
...
設定32 x 32像素的「播放/暫停」按鈕，並從控制欄的上邊緣和左邊緣定位6個像素。 最後，當選擇或未選擇時，為四個不同的按鈕狀態中的每個狀態顯示不同的影像。
