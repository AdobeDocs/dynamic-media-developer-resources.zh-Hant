...
>[!NOTE]
>
>此按鈕支援`state`屬性選取器和選取的屬性選取器，它們可用來將不同的外觀元素套用至不同的按鈕狀態。 特別是，`selected='true'`對應於顯示註解時的狀態，而隱藏註解時使用`selected='false'`。
...
範例 — 設定28 x 28畫素的「隱藏式字幕」按鈕，其位置從上到下皆為4畫素，從控制列的右邊緣皆為68畫素。 最後，選取或未選取時，針對四種不同按鈕狀態分別顯示不同的影像。
