---
description: 混合媒體檢視器的所有視覺化自訂和大部分行為自訂，都是透過建立自訂CSS來完成。
keywords: 回應式
...
混合媒體檢視器的所有視覺化自訂和大部分行為自訂，都是透過建立自訂CSS來完成。
...
建立自訂CSS時，請記得檢視器會將`.s7mixedmediaviewer`類別指派給其容器DOM元素。 如果您使用使用`style=`命令傳遞的外部CSS檔案，請在CSS規則的子體選取器中使用`.s7mixedmediaviewer`類作為父類。 如果您要在網頁上執行內嵌樣式，請額外以容器DOM元素的ID限定此選取器，如下所示：
