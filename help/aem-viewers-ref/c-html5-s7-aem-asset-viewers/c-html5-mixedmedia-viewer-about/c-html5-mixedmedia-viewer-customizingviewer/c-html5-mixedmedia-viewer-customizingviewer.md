...
建立自訂CSS即可完成混合媒體檢視器的所有視覺化自訂和大部分行為自訂。
...
建立自訂CSS時，請記住檢視器會將類 `.s7mixedmediaviewer` 別指派給其容器DOM元素。 如果您使用隨命令傳遞的外部CSS檔案， `style=` 請在CSS規 `.s7mixedmediaviewer` 則的子體選擇器中，使用類別做為上層類別。 如果您要在網頁上執行內嵌樣式，請另外以容器DOM元素的ID來限定此選取器，如下所示：
