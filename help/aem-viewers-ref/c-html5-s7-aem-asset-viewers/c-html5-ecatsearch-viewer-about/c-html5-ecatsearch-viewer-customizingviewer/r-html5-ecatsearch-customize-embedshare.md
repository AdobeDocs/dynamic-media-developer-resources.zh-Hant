...
範例 — 若要設定28 x 28畫素的內嵌「共用」按鈕，並針對四種不同按鈕狀態分別顯示不同影像：
...
範例 — 若要設定包含填補的對話方塊標題，請以24 x 14畫素圖示及粗體16點標題顯示。 最後，還有28 x 28畫素的「關閉」按鈕，從上方放置兩個畫素，從對話方塊容器右側放置兩個畫素：
...
>[!NOTE]
>
>下拉式方塊支援`expanded`屬性選擇器，可能的值為`true`和`false`。 當下拉式方塊顯示預先定義的其中一個內嵌大小時，會使用值`true`，因此應採用所有可用的寬度。 在下拉式方塊中選取自訂大小選項時，會使用值`false`，因此它應該縮小以留出空間給自訂寬度和高度輸入欄位。
