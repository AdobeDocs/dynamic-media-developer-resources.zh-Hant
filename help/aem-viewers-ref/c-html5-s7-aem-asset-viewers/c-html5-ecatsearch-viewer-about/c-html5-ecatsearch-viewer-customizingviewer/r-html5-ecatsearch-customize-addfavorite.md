...
>[!NOTE]
>
>此按鈕支援 `state` 和 `selected` 屬性選擇器，可用於將不同的外觀應用於不同的按鈕狀態。 特別是， `selected='true'` 對應於用戶通過選擇或點擊添加新「收藏夾」表徵圖時的狀態。 然而 `selected='false'` 當用戶可以縮放、平移和交換頁面時，對應於正常操作模式。
...
示例 — 設定「添加收藏夾」按鈕，該按鈕為28 x 28像素，並在選中或未選中時為四個不同按鈕狀態中的每個狀態顯示不同的影像。
