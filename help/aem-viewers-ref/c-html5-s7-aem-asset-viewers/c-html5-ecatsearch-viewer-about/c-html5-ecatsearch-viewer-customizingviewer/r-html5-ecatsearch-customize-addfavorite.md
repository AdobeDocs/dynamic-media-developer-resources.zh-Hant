...
>[!NOTE]
>
>此按鈕同時支援 `state` 和 `selected` 屬性選取器，可將不同的外觀元素套用至不同的按鈕狀態。 尤其是， `selected='true'` 對應於使用者可透過選取或點選來新增最愛圖示時的狀態。 而 `selected='false'` 對應於使用者可以縮放、平移和交換頁面時的正常操作模式。
...
範例 — 設定28 x 28畫素的「新增我的最愛」按鈕，並在選取或未選取時，針對四種不同按鈕狀態顯示不同的影像。
