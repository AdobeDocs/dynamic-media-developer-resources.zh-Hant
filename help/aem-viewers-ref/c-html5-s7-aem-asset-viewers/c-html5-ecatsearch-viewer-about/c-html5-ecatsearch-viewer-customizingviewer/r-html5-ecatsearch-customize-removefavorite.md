---
description: 「移除我的最愛」按鈕的位置完全由「我的最愛」功能表管理。
seo-description: 「移除我的最愛」按鈕的位置完全由「我的最愛」功能表管理。
seo-title: 「移除我的最愛」按鈕
...
# 「移除我的最愛」按鈕{#remove-favorite-button}

「移除我的最愛」按鈕的位置完全由「我的最愛」功能表管理。

<!--<a id="section_061E550C1C1D4DB2BD663A898895B38C"></a>-->

「移除我的最愛」按鈕的外觀會由下列CSS類別選擇器控制：
...
>[!NOTE]
>
>此按鈕同時支援 `state` 和屬 `selected` 性選擇器，可用來將不同的外觀套用至不同的按鈕狀態。 尤其是， `selected='true'` 當使用者可以按一下或點選新增「我的最愛」圖示時，會對應至狀態。 `selected='false'` 對應於使用者可縮放、平移和交換頁面時的一般操作模式。
...
範例——設定「移除我的最愛」按鈕（28 x 28像素），並在選取或未選取時，針對四個不同按鈕狀態的每個狀態顯示不同的影像。
