---
title: 主控制列
description: 主要控制列是桌上型電腦系統和平板電腦上的矩形區域，其中包含所有可用於eCatalog搜尋檢視器的使用者介面控制項（「大頁面」按鈕除外）。
...
主要控制列是桌上型電腦系統和平板電腦上的矩形區域，其中包含所有可用於eCatalog搜尋檢視器的使用者介面控制項（「大頁面」按鈕除外）。
...
>[!NOTE]
>
>此按鈕支援`state`和`selected`屬性選取器，它們可用來將不同的外觀元素套用至不同的按鈕狀態。 特別是，`state="selected"`對應於初始捲動按鈕狀態（當可以捲動控制列內容至左側時）。 `state="default"`對應於內容一直向左捲動時的狀態，而捲動按鈕建議將其恢復為初始狀態。
