...
>[!NOTE]
>
>此按鈕支援`state`和`selected`屬性選取器，它們可用來將不同的外觀元素套用至不同的按鈕狀態。
>
>特別是，`selected='false'`對應於初始捲動按鈕狀態，`selected='true'`對應於搜尋面板作用中的狀態。
...
範例 — 設定28 x 28畫素的「搜尋」按鈕，並在選取或未選取時，針對四種不同按鈕狀態分別顯示不同的影像。
