---
title: 大型上一頁按鈕
description: 選取此按鈕會將使用者前往目錄中的上一頁。 此按鈕出現在主控制欄中。 行動電話上不會顯示此按鈕以儲存螢幕空間。 您可以使用CSS來調整此按鈕的大小、外觀和位置。
...
# 大型上一頁按鈕{#large-previous-page-button}

選取此按鈕會將使用者前往目錄中的上一頁。 此按鈕出現在主控制欄中。 行動電話上不會顯示此按鈕以儲存螢幕空間。 您可以使用CSS來調整此按鈕的大小、外觀和位置。
...
範例：若要設定一個大型的上一頁按鈕，其長度為56 x 56像素，垂直居中並錨定至左側檢視器邊框，並針對四個不同按鈕狀態中的每個狀態顯示不同的影像。
