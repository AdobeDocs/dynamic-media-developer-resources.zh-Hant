...
seo-description: 按一下或點選此按鈕可讓使用者進入目錄的上一頁。 此按鈕會出現在主控制列中。 行動電話上不會顯示此按鈕來儲存螢幕空間。 您可以使用CSS來調整此按鈕的大小、外觀和位置。
seo-title: 大型上一頁按鈕
...
範例——若要設定56 x 56像素、垂直置中並錨定至左側檢視器邊框的大型上一頁按鈕，並針對四個不同的按鈕狀態顯示不同的影像。
