---
title: 「上一頁」按鈕
description: 選擇此按鈕將用戶帶到目錄中的上一頁。 此按鈕顯示在主控制欄中。 此按鈕不顯示在手機上以保存螢幕不動產。 可以使用CSS調整此按鈕的大小、外觀和位置。
...
選擇此按鈕將用戶帶到目錄中的上一頁。 此按鈕顯示在主控制欄中。 此按鈕不顯示在手機上以保存螢幕不動產。 可以使用CSS調整此按鈕的大小、外觀和位置。
...
示例 — 設定一個上一頁按鈕，該按鈕為28 x 28像素，從主控制欄的底部放置4個像素，從右邊放置250個像素。 最後，顯示四個不同按鈕狀態中每個狀態的不同影像。
