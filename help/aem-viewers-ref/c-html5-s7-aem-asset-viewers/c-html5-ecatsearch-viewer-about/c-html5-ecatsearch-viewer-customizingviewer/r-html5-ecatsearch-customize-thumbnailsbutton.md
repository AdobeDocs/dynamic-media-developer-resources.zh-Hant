---
title: 縮略圖按鈕
description: 選擇此按鈕將在主視圖和縮略圖之間切換查看器。 此按鈕顯示在主控制欄中。 可以使用CSS調整此按鈕的大小、外觀和位置。
...
# 縮略圖按鈕{#thumbnails-button}

選擇此按鈕將在主視圖和縮略圖之間切換查看器。 此按鈕顯示在主控制欄中。 可以使用CSS調整此按鈕的大小、外觀和位置。
...
>[!NOTE]
>
>此按鈕支援 `state` 和 `selected` 屬性選擇器，可用於將不同的外觀應用於不同的按鈕狀態。 特別是， `selected='true'` 與縮略圖模式處於活動狀態時的查看器狀態相對應， `selected='false'` 與主視圖的預設狀態相對應。
...
示例 — 設定28 x 28像素的縮略圖按鈕，並從主控制欄的底部放置4個像素，從左邊緣放置5個像素。 最後，當選擇或未選擇時，為四個不同的按鈕狀態中的每個狀態顯示不同的影像。
