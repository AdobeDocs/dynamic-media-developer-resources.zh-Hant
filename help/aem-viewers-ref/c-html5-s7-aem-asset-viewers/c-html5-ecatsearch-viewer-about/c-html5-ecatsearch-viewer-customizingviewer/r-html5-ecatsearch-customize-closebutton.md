---
title: 關閉按鈕
description: 按一下或點選此按鈕會關閉內含網頁。 只有在closebutton引數設定為1時，此按鈕才會出現。 此按鈕在桌上型電腦系統上無法使用。 您可以使用CSS調整此按鈕的大小、外觀和位置。
...
範例 — 設定56 x 56畫素的「關閉」按鈕，並從主控制列的頂端和右側邊緣放置4個畫素。 最後，針對四種不同的按鈕狀態分別顯示不同的影像。
