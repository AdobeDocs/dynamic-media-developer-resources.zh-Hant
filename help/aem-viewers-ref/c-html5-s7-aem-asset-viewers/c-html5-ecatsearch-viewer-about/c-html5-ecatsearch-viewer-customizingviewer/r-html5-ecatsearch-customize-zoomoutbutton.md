---
title: 縮小按鈕
description: 選取此按鈕會放大主檢視中的影像。 行動電話上不會顯示此按鈕以儲存螢幕空間。 您可以使用CSS來調整此按鈕的大小、外觀和位置。
...
選取此按鈕會放大主檢視中的影像。 行動電話上不會顯示此按鈕以儲存螢幕空間。 您可以使用CSS來調整此按鈕的大小、外觀和位置。
...
範例 — 設定28 x 28像素的縮小按鈕，並從底部放置4像素，從主控制條的右邊放置75像素。 最後，針對四個不同的按鈕狀態顯示不同的影像。
