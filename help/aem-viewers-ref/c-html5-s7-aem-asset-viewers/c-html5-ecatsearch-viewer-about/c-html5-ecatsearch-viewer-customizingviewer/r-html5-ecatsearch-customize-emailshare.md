...
對話方塊標題由圖示、標題文字和「關閉」按鈕組成。 標題容器由下列CSS類別選取器控制
...
「關閉」按鈕工具提示和對話方塊標題可以本地化。 如需詳細資訊，請參閱[使用者介面元素的本地化](../../../c-html5-s7-aem-asset-viewers/c-html5-ecatsearch-viewer-about/c-html5-ecatsearch-viewer-localization.md#concept-cbfc39344c494eb7b9f6a272cff0cc74)。

範例 — 若要設定具有填補的對話方塊標題，請以24 x 17畫素圖示及粗體16點標題來設定。 最後，還有28 x 28畫素的「關閉」按鈕，它定位在對話方塊容器上方的兩個畫素和右方的兩個畫素：
...
對話方塊頁尾包含取消和傳送電子郵件按鈕。 頁尾容器由下列CSS類別選取器控制：
...
此按鈕的工具提示可本地化。 如需詳細資訊，請參閱[使用者介面元素的本地化](../../../c-html5-s7-aem-asset-viewers/c-html5-ecatsearch-viewer-about/c-html5-ecatsearch-viewer-localization.md#concept-cbfc39344c494eb7b9f6a272cff0cc74)。
...
主要對話方塊區域位於頁首和頁尾之間，右側包含可捲動對話方塊內容和捲動面板。 在任何情況下，元件都會管理此區域的寬度，無法在CSS中進行設定。 主要對話方塊區域可使用下列CSS類別選取器來控制：
...
>[!NOTE]
>
>主要對話方塊區域支援選用的`state`屬性選取器。 當電子郵件表單提交且對話方塊顯示確認訊息時，此欄位會設為`sendsuccess`。 只要確認訊息很小，就會在顯示此類確認訊息時，使用此屬性選取器來降低對話方塊高度。
...
對話方塊標籤可以本地化。 如需詳細資訊，請參閱[使用者介面元素的本地化](../../../c-html5-s7-aem-asset-viewers/c-html5-ecatsearch-viewer-about/c-html5-ecatsearch-viewer-localization.md#concept-cbfc39344c494eb7b9f6a272cff0cc74)。
...
>[!NOTE]
>
>輸入欄位容器支援選用的`state`屬性選取器。 當使用者在輸入資料格式中發生錯誤且內嵌驗證失敗時，其會設為`verifyerror`。 此屬性選取器可用來反白顯示表單中錯誤的使用者輸入。

從左上至對話方塊內文右邊緣的標籤展開的大多數輸入欄位（包括「寄件者」欄位和「訊息」欄位）都是使用下列CSS類別選取器來控制：
...
[收件者]欄位較窄，因為它為右側的移除電子郵件按鈕分配了空間。 若要控制CSS類別，請使用下列CSS類別選取器：
...
範例 — 將表單設定為有一個畫素灰色邊框，在所有輸入欄位周圍有九個畫素的內距。 對於驗證失敗的欄位，使用相同的紅色邊框。 最後，要有250畫素寬的終止欄位，其餘的輸入欄位有300畫素寬的終止欄位：
...
範例 — 若要將電子郵件訊息設定為50畫素高，並使用`break-word`個自動換行：
...
要共用的內容會顯示在對話方塊內文的底部，並包含縮圖、標題、原始URL和說明。 它會包裝到容器中，並由下列CSS類別選取器控制：
...
當使用者輸入錯誤的輸入資料且內嵌驗證失敗時，對話方塊內文的頂端會顯示訊息。 在提交表單時，當對話方塊必須呈現錯誤或確認訊息時，也會顯示此訊息。 若要控制CSS類別，請使用下列CSS類別選取器：
...
>[!NOTE]
>
>此訊息支援具有以下可能值的`state`屬性選擇器： `verifyerror`、`senderror`和`sendsuccess`。 當訊息因內嵌輸入驗證失敗而顯示時，會設定值`verifyerror`。 當後端電子郵件服務回報錯誤時，會設定值`senderror`。 成功傳送電子郵件時設定值`sendsuccess`。 如此一來，即可根據對話方塊狀態，以不同方式設定訊息樣式。

按鈕工具提示可以本地化。

如需詳細資訊，請參閱[使用者介面元素的本地化](../../../c-html5-s7-aem-asset-viewers/c-html5-ecatsearch-viewer-about/c-html5-ecatsearch-viewer-localization.md#concept-cbfc39344c494eb7b9f6a272cff0cc74)。

範例 — 若要設定訊息以使用10點粗體字型，請在左側使用25畫素行高和20畫素邊框間距。 最後，使用驚歎號圖示，如果有錯誤則使用紅色文字，如果有成功則使用無圖示和綠色文字：
