---
description: 按一下或點選此按鈕，即可將使用者帶至目錄的最後一頁。 此按鈕顯示在案頭系統和平板電腦的主控制欄中；在行動電話上，它會新增至次要控制列。 您可以使用CSS來調整此按鈕的大小、外觀和位置。
...
按一下或點選此按鈕，即可將使用者帶至目錄的最後一頁。 此按鈕顯示在案頭系統和平板電腦的主控制欄中；在行動電話上，它會新增至次要控制列。 您可以使用CSS來調整此按鈕的大小、外觀和位置。
...
範例：若要設定最後一個頁面按鈕，即28 x 28像素，從底部放置4像素，從主控制列的左邊緣放置220像素，並針對四個不同按鈕狀態中的每一個顯示不同的影像。
