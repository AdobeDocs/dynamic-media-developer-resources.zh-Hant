...
## 使用eCatalog搜尋檢視器{#section-eb575084a99647c3a9591f439f40b412}的社交媒體分享工具
...
## 內嵌eCatalog Search Viewer {#section-6bb5d3c502544ad18a58eafe12a13435}
...
建議您使用現成可用的HTML頁面來進行快顯作業模式。 在此例中，它稱為[!DNL eCatalogSearchViewer.html]，位於標準IS-Viewer部署的[!DNL html5/]子資料夾中：
...
1. 將檢視器JavaScript檔案新增至您的網頁。
   ...
   建立檢視器時，您必須在HTML標題中新增指令碼標籤。 請務必加入[!DNL eCatalogSearchViewer.js]，才能使用檢視器API。 [!DNL eCatalogSearchViewer.js]檔案位於標準IS-Viewer部署的[!DNL html5/js/]子資料夾下：
...
1. 初始化檢視器。
   ...
   以下是建立檢視器例項、將最小必要組態選項傳遞至建構函式，以及呼叫`init()`方法的範例。 範例假設`eCatalogSearchViewer`為檢視器例項；`s7viewer`是佔位符`DIV`的名稱；`http://s7d1.scene7.com/is/image/`是影像伺服URL，而`Viewers/Pluralist`是資產：
