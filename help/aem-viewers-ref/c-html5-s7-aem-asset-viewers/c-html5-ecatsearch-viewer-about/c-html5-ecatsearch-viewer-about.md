---
description: eCatalog Search Viewer是一個目錄查看器，它以分頁或分頁方式以分頁方式顯示電子手冊。eCatalog允許用戶使用其他用戶介面元素或專用縮略圖模式瀏覽目錄。 用戶還可以放大每頁，以獲得更詳細的資訊。
keywords: 響應
...
# 電子目錄搜索{#ecatalog-search}

eCatalog Search Viewer是一個目錄查看器，它以分頁或分頁方式以分頁方式顯示電子手冊。eCatalog允許用戶使用其他用戶介面元素或專用縮略圖模式瀏覽目錄。 用戶還可以放大每頁，以獲得更詳細的資訊。

此查看器可以與對話框配合使用，並支援可選的影像映射和社交共用工具。 它具有縮放工具、目錄導航工具、全屏支援、縮略圖和可選的關閉按鈕。 查看器還支援社交共用工具、打印、下載和收藏夾。 它設計用於台式機和移動設備。

用戶還可以對目錄內容進行基於關鍵字或基於短語的搜索。
...
eCatalog Search Viewer表示主JavaScript檔案和一組幫助程式檔案（單個JavaScript包含由查看器在運行時下載的由此特定查看器、資產和CSS使用的所有查看器SDK元件）

您可以使用隨IS-Viewer提供的生產就緒型HTML頁或嵌入式模式在彈出模式下使用eCatalog Search Viewer，在該模式下，它使用文檔化的API整合到目標網頁中。
...
## 與eCatalog Search Viewer交互 {#section-642e66ca38cd4032992840ec6c0b0cd2}

eCatalog Search Viewer支援其他移動應用程式中常見的以下觸控手勢。
...
## 使用eCatalog Search Viewer的社交媒體共用工具 {#section-eb575084a99647c3a9591f439f40b412}

eCatalog Search Viewer支援社交共用工具。它們作為主控制欄中的按鈕可用，當用戶按一下或輕擊該按鈕時，該按鈕將擴展為共用工具欄。
...
查看器的「搜索」功能在主工具欄中可用作外觀玻璃表徵圖。 按一下或輕擊表徵圖將激活帶輸入欄位的「搜索」面板。 在輸入關鍵字或短語並按Enter後，查看器將在面板中呈現搜索結果，並在主視圖中加亮找到的字詞。
...
建議您使用現成HTML頁面進行彈出操作模式。 在這種情況下，它叫 [!DNL eCatalogSearchViewer.html] 位於 [!DNL html5/] 標準IS查看器部署的子資料夾：
...
當查看器在初始載入後不更改其大小時，使用固定大小嵌入。 這是具有靜態佈局的網頁的最佳選擇。

響應性設計嵌入假定查看器可能需要在運行時調整大小以響應其容器的大小變化 `DIV`。 最常見的用例是向使用靈活頁面佈局的網頁添加查看器。

在響應性設計嵌入模式中，查看器根據網頁大小其容器的方式而具有不同的行為 `DIV`。 如果網頁僅設定容器的寬度 `DIV`在保持高度不受限制的情況下，觀看者根據所使用資產的縱橫比自動選擇其高度。 此功能可確保資產完美地貼入視圖中，而不會在側面出現任何填充。 此使用案例是使用響應性佈局框架(如Bootstrap、基礎等)的網頁中最常見的使用案例。
...
1. 將查看器JavaScript檔案添加到網頁。
   ...
   建立查看器要求在HTML頭中添加指令碼標籤。 在使用查看器API之前，請確保包括 [!DNL eCatalogSearchViewer.js]。 的 [!DNL eCatalogSearchViewer.js] 檔案位於 [!DNL html5/js/] 標準IS查看器部署的子資料夾：
...
1. 設定查看器大小
   ...
   可以通過為聲明查看器來設定其靜態大小 `.s7ecatalogsearchviewer` 頂級CSS類（以絕對單位表示），或使用 `stagesize` 修改量。

   您可以將大小調整直接放在CSS中的HTML頁面上，也可以放在自定義查看器CSS檔案中，該檔案稍後將分配給Dynamic Media Classic的查看器預設記錄，或使用style命令顯式傳遞。
   ...
   可以設定 `stagesize` 修飾符，在Dynamic Media Classic的查看器預設記錄中，或用查看器初始化代碼將其顯式傳遞 `params` 或作為API調用（如「命令參考」部分中所述），如下所示：
...
1. 正在初始化查看器。
   ...
   完成上述步驟後，將建立 `s7viewers.eCatalogSearchViewer` 類，將所有配置資訊傳遞給其建構子和調用 `init()` 的子常式。 配置資訊作為JSON對象傳遞給建構子。 至少，此對象具有 `containerId` 包含查看器容器ID和嵌套名稱的欄位 `params` 具有查看器支援的配置參數的JSON對象。 在這個例子中， `params` 對象必須至少將Image Serving URL傳遞為 `serverUrl` 及初始資產 `asset` 的下界。 基於JSON的初始化API允許您使用單行代碼建立和啟動查看器。

   必須將查看器容器添加到DOM中，以便查看器代碼可以通過其ID查找容器元素。 某些瀏覽器將生成DOM延遲到網頁結束。 但是，要獲得最大相容性，請調用 `init()` 方法 `BODY` 標籤，或者身上 `onload()` 的子菜單。

   同時，容器元素不一定只是網頁佈局的一部分。 例如，它可能隱藏在 `display:none` 指定的樣式。 在這種情況下，查看器將其初始化過程延遲到網頁將容器元素帶回佈局的那一刻。 發生此情況時，查看器載入將自動恢復。

   以下是建立查看器實例、將最小必要配置選項傳遞給建構子並調用 `init()` 的雙曲餘切值。 該示例假定 `eCatalogSearchViewer` 是查看器實例； `s7viewer` 是佔位符的名稱 `DIV`; `https://s7d1.scene7.com/is/image/` 是影像服務URL, `Viewers/Pluralist` 是資產：
   ...
   以下代碼是嵌入固定大小的eCatalog Search Viewer的普通網頁的完整示例：
...
在定義了寬度和高度的柔性尺寸嵌入的情況下，網頁的樣式是不同的。 即，它為「保持器」提供兩種大小 `DIV` 並將其置於瀏覽器窗口中。 此外，網頁還設定 `HTML` 和 `BODY` 元素到100%:
