...
title: 輔助技術支援
feature: Dynamic Media經典，檢視器，SDK/API,eCatalog搜尋，協助工具
role: 開發人員，商業從業人員
...
搜尋使用者介面會分組在角色為`search`的元素中。 搜索輸入欄位具有`searchbox`角色，並引用由`SearchPanel.INFO_PROMPT`本地化符號控制的資訊標籤和`aria-describedby`屬性。
