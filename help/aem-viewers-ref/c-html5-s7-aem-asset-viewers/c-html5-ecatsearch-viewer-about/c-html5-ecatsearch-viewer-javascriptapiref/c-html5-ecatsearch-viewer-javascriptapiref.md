...
description: eCatalog搜尋檢視器的主要類別是eCatalogSearchViewer。 這會在s7viewers名稱空間中宣告。 此JavaScript API涵蓋此特定類別的建構函式、方法和回呼。
...
eCatalog搜尋檢視器的主要類別是eCatalogSearchViewer。 這會在s7viewers名稱空間中宣告。 此JavaScript API涵蓋此特定類別的建構函式、方法和回呼。

在下列所有範例中，`<instance>`代表從`s7viewers.eCatalogSearchViewer`類別具現化之JavaScript檢視器物件的實際名稱。
