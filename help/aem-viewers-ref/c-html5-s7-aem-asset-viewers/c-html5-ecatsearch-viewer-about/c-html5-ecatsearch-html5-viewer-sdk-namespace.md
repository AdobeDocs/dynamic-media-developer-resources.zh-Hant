...
檢視器用來載入及初始化SDK元件的名稱空間取決於檢視器運作的環境。 如果檢視器是在AEM (Adobe Experience Manager)中執行，檢視器會將SDK元件載入`s7viewers.s7sdk`名稱空間。 Dynamic Media Classic提供的檢視器會將SDK載入`s7classic.s7sdk`。
