---
title: 關閉按鈕
description: 選取此按鈕會關閉內含的網頁。 只有在closebutton引數設定為1時，此按鈕才會出現。 您可以使用CSS調整此按鈕的大小、外觀和位置。
...
按一下或點選此按鈕會關閉內含網頁。 只有在closebutton引數設定為1時，此按鈕才會出現。 您可以使用CSS調整此按鈕的大小、外觀和位置。
...
範例 — 設定32 x 32畫素的「關閉」按鈕，並從檢視器的上邊緣與右邊緣放置6個畫素。 它必須針對四種不同的按鈕狀態分別顯示不同的影像。
