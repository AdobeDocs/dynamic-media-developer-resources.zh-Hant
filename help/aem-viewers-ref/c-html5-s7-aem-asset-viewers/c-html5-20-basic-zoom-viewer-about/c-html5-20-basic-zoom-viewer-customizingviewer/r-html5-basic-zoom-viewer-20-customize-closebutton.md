---
title: 關閉按鈕
description: 選擇此按鈕將關閉包含的網頁。 僅當關閉按鈕參數設定為1時，此按鈕才會出現。 可以使用CSS調整此按鈕的大小、外觀和位置。
...
按一下或點擊此按鈕可關閉包含的網頁。 僅當關閉按鈕參數設定為1時，此按鈕才會出現。 可以使用CSS調整此按鈕的大小、外觀和位置。
...
示例 — 設定一個32 x 32像素的「關閉」按鈕，並從查看器的上邊緣和右邊緣定位六個像素。 它必須為四個不同按鈕狀態中的每個狀態顯示不同的影像。
