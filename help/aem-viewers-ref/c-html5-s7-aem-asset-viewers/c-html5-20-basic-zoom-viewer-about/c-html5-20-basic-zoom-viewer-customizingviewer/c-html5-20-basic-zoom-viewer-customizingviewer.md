...
建立自訂CSS時，請記住檢視器會將類 `.s7basiczoomviewer` 別指派給其容器DOM元素。 如果您使用隨命令傳遞的外部CSS檔案， `style=` 請在CSS規 `.s7basiczoomviewer` 則的子體選擇器中，使用類別做為上層類別。 如果您要在網頁上執行內嵌樣式，請另外以容器DOM元素的ID來限定此選取器，如下所示：
...
第二組CSS標籤包含 `.s7mouseinput` 和 `.s7touchinput`。 `.s7touchinput` 如果當前設備具有觸摸輸入功能，則設定此選項；否則， `.s7mouseinput` 將使用。 這些標籤主要用於針對不同的輸入類型建立具有不同螢幕大小的使用者介面輸入元素，因為觸控輸入通常需要較大的元素。 當裝置同時具備滑鼠輸入和觸控功能時， `.s7touchinput` 會設定檢視器並呈現適合觸控的使用者介面。
