---
title: 自定義基本縮放查看器
description: 「基本縮放查看器」的所有可視化自定義和大多數行為自定義都通過建立自定義CSS來完成。
keywords: 響應
...
# 自定義基本縮放查看器{#customizing-basic-zoom-viewer}

「基本縮放查看器」的所有可視化自定義和大多數行為自定義都通過建立自定義CSS來完成。
...
為查看器提供了兩個現成的CSS檔案，用於「淺」和「暗」顏色方案。 預設情況下使用&quot;light&quot;版本，但您可以使用以下標準CSS切換到&quot;dark&quot;版本：
...
建立自定義CSS時，切記查看器指定 `.s7basiczoomviewer` 類到其容器DOM元素。 如果使用與 `style=` 命令，使用 `.s7basiczoomviewer` 類作為CSS規則的後代選擇器中的父類。 如果要在網頁上執行嵌入樣式，還應使用容器DOM元素的ID來限定此選擇器，如下所示：

`#<containerId>.s7basiczoomviewer`

## 構建響應設計CSS {#section-0bb49aca42d242d9b01879d5ba59d33b}

有可能針對不同的設備並在CSS中嵌入大小，以根據用戶的設備或特定網頁佈局使內容顯示方式不同。 此目標包括但不限於不同佈局、用戶介面元素大小和圖稿解析度。
...
第二組CSS標籤包含 `.s7mouseinput` 和 `.s7touchinput`。 的 `.s7touchinput` 如果當前設備具有觸摸輸入功能，則設定標籤；否則， `.s7mouseinput` 的子菜單。 這些標籤主要用於為不同的輸入類型建立具有不同螢幕大小的用戶介面輸入元素，因為觸摸輸入通常需要較大的元素。 如果設備同時具備滑鼠輸入和觸摸功能， `.s7touchinput` 設定，並且查看器呈現一個觸摸友好用戶介面。

下面的示例CSS將滑鼠輸入系統上的按鈕放大大小設定為28 x 28像素，將觸摸設備上的按鈕放大大小設定為56 x 56像素。 此外，如果查看器大小變小，它會完全隱藏按鈕：
...
使用CSS標籤是構建響應性設計的CSS的最靈活方法。 原因是它不僅允許您針對設備螢幕大小，還允許您針對實際的查看器大小，這對於響應性設計的佈局非常有用。

可以將預設查看器CSS檔案用作CSS標籤方法的示例。

**CSS媒體查詢**

您還可以使用純CSS媒體查詢來完成設備檢測。 僅當給定媒體查詢塊在相應設備上運行時，才應用該塊中包含的所有內容。
