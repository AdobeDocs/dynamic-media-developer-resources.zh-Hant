...
1. 建立和初始化檢視器。
   ...
   以下是建立檢視器例項、將最小必要設定選項傳遞至建構函式並呼叫方法的 `init()` 範例。 範例假設 `basicZoomViewer` 是檢視器例項； `s7viewer` 是佔位符的名稱 `DIV`; `http://s7d1.scene7.com/is/image/` 是影像伺服URL, `Scene7SharedAssets/Backpack_B` 且是資產：
