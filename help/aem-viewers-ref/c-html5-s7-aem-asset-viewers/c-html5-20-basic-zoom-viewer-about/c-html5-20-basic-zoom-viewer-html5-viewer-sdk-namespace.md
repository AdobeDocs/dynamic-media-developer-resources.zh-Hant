...
檢視器用來載入及初始化SDK元件的名稱空間取決於檢視器運作的環境。 如果檢視器在Adobe Experience Manager中執行，檢視器會將SDK元件載入`s7viewers.s7sdk`名稱空間。 Dynamic Media Classic提供的檢視器會將SDK載入`s7classic.s7sdk`。
...
例如，如果您打算接聽`StatusEvent.NOTF_VIEW_READY`事件，而Experience Manager已提供檢視器，則完整事件型別為`s7viewers.s7sdk.event.StatusEvent.NOTF_VIEW_READY`，而事件接聽程式程式碼看起來類似下列：
