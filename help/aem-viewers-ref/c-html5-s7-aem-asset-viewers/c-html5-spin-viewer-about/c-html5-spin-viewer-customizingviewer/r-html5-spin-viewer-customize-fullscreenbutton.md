---
description: 當使用者按一下，此按鈕會使檢視器進入或退出全螢幕模式。 如果檢視器在快顯模式中運作，且系統不支援原生全螢幕，則不會顯示此按鈕。 您可以使用CSS來調整此按鈕的大小、外觀和位置。
...
當使用者按一下，此按鈕會使檢視器進入或退出全螢幕模式。 如果檢視器在快顯模式中運作，且系統不支援原生全螢幕，則不會顯示此按鈕。 您可以使用CSS來調整此按鈕的大小、外觀和位置。
