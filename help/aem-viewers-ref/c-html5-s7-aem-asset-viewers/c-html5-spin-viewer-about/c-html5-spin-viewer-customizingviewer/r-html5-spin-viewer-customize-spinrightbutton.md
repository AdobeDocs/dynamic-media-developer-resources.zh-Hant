---
title: 向右迴轉按鈕
description: 按一下或點選此按鈕，即可在主檢視的右側旋轉影像。 為了節省熒幕空間，此按鈕不會顯示在行動電話上。 此外，使用多維度迴轉集時會隱藏按鈕。 您可以使用CSS調整按鈕的大小、外觀和位置。
...
按一下或點選此按鈕，即可在主檢視的右側旋轉影像。 為了節省熒幕空間，此按鈕不會顯示在行動電話上。 此外，使用多維度迴轉集時會隱藏按鈕。 您可以使用CSS調整按鈕的大小、外觀和位置。
...
範例 — 設定28 x 28畫素的右迴轉按鈕，該按鈕位於內部容器的右邊緣。 最後，針對四種不同按鈕狀態分別顯示不同影像：
