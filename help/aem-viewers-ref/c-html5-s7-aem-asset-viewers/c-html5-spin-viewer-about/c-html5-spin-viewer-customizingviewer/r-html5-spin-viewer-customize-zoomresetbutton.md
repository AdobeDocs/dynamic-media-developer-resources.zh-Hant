---
title: 縮放重置按鈕
description: 按一下或點擊此按鈕將重置主視圖中的影像。 可以使用CSS調整此按鈕的大小、外觀和位置。
...
按一下或點擊此按鈕將重置主視圖中的影像。 可以使用CSS調整此按鈕的大小、外觀和位置。
...
示例 — 設定32 x 32像素的縮放重置按鈕，並從查看器的上邊緣和右邊緣定位6個像素。 最後，顯示四個不同按鈕狀態中每個狀態的不同影像。
