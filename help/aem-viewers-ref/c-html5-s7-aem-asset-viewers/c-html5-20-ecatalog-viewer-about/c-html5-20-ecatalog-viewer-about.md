...
1. 正在初始化查看器。
   ...
   以下是建立檢視器例項、將最低必要的設定選項傳遞至建構函式，並呼叫 `init()` 方法。 此範例假設 `eCatalogViewer` 是檢視器例項； `s7viewer` 是佔位符的名稱 `DIV`; `https://s7d1.scene7.com/is/image/` 是影像伺服URL，且 `Viewers/Pluralist` 是資產：
