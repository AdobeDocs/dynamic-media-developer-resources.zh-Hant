...
此檢視器可完全以鍵盤存取，如鍵盤存取 [和導覽中所述](../../c-keyboard-accessibility.md#topic-f5650e9493404e55a3627c8d1366b861)。
...
建議您使用現成可用的HTML頁面來進行快顯作業模式。 在這種情況下，它會被呼叫， [!DNL eCatalogViewer.html] 並位於標準IS- [!DNL html5/] Viewer部署的子資料夾中：
...
1. 將檢視器JavaScript檔案新增至您的網頁。
   ...
   建立檢視器時，您必須在HTML標題中新增指令碼標籤。 在您使用檢視器API之前，請確定您已加入 [!DNL eCatalogViewer.js]。 此檔 [!DNL eCatalogViewer.js] 案位於標準IS- [!DNL html5/js/] Viewer部署的子資料夾下：
...
1. 設定檢視器大小
   ...
   您可以設定檢視器的靜態大小，方法是以絕對單位 `.s7ecatalogviewer` 來宣告頂層CSS類別，或使用修飾 `stagesize` 元。
...
1. 初始化檢視器。
   ...
   完成上述步驟後，您將建立類的實例，將所有配置資訊 `s7viewers.eCatalogViewer` 傳遞給其建構子，並在查看器實例 `init()` 上調用方法。 設定資訊會以JSON物件的形式傳遞至建構函式。 此物件至少會有欄位， `containerId` 其中包含檢視器容器ID名稱和內嵌 `params` JSON物件，以及檢視器支援的設定參數。 在此情況下，物 `params` 件必須至少將「影像伺服URL」傳遞為屬 `serverUrl` 性，並將初始資產傳遞為 `asset` 參數。 以JSON為基礎的初始化API可讓您使用單行程式碼來建立和啟動檢視器。
