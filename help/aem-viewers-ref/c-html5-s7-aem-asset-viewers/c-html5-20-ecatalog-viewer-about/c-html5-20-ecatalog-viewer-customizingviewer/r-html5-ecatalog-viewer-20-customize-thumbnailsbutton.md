---
title: 縮略圖按鈕
description: 按一下或點擊此按鈕將在主視圖和縮略圖之間切換查看器。 此按鈕顯示在主控制欄中。 可以使用CSS調整此按鈕的大小、外觀和位置。
...
選擇或點擊此按鈕會在主視圖和縮略圖之間切換查看器。 此按鈕顯示在主控制欄中。 可以使用CSS調整此按鈕的大小、外觀和位置。
...
示例 — 設定縮略圖按鈕，該按鈕為28 x 28像素，從底部放置4個像素，從主控制欄的左邊緣放置5個像素。 最後，當選擇或未選擇時，為四個不同的按鈕狀態中的每個狀態顯示不同的影像。
