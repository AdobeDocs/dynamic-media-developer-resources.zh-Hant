...
# 刪除收藏夾按鈕{#remove-favorite-button}

「刪除收藏夾」按鈕的位置由「收藏夾」菜單完全管理。

<!--<a id="section_061E550C1C1D4DB2BD663A898895B38C"></a>-->

使用以下CSS類選擇器控制「刪除收藏夾」按鈕的外觀：
...
>[!NOTE]
>
>此按鈕支援 `state` 和 `selected` 屬性選取器，可用來將不同外觀套用至不同按鈕狀態。 特別是， `selected='true'` 對應至使用者按一下或點選以新增「最喜愛」圖示的狀態。 屬性 `selected='false'` 與使用者可縮放、平移和交換頁面時的一般操作模式相對應。
...
範例：設定一個28 x 28像素的「移除最愛」按鈕，並在選取或未選取時，針對四個不同按鈕狀態中的每個狀態顯示不同的影像。
