...
>[!NOTE]
>
>此按鈕同時支援`state`和`selected`屬性選取器，它們可用來將不同的外觀元素套用至不同的按鈕狀態。 特別是，`selected='true'`對應於使用者可以按一下或點選以新增最愛圖示的狀態。 當使用者可以縮放、平移和交換頁面時，屬性`selected='false'`會對應到一般操作模式。
...
範例 — 設定28 x 28畫素的「移除我的最愛」按鈕，並在選取或未選取時，針對四種不同的按鈕狀態分別顯示不同的影像。
