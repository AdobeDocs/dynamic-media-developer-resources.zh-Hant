...
範例 — 設定28 x 28畫素的電子郵件共用按鈕，並針對四種不同按鈕狀態分別顯示不同影像。
...
「關閉」按鈕工具提示和對話方塊標題可以本地化。 如需詳細資訊，請參閱[使用者介面元素的本地化](../../../c-html5-s7-aem-asset-viewers/c-html5-20-ecatalog-viewer-about/c-html5-20-ecatalog-viewer-localization.md#concept-cbfc39344c494eb7b9f6a272cff0cc74)。

範例 — 若要設定包含邊框間距的對話方塊標題，請以24 x 17畫素圖示和16點粗體標題顯示。 最後，還有28 x 28畫素的「關閉」按鈕，此按鈕位於對話方塊容器上方的兩個畫素與右方的兩個畫素：
...
此按鈕的工具提示可本地化。 如需詳細資訊，請參閱[使用者介面元素的本地化](../../../c-html5-s7-aem-asset-viewers/c-html5-20-ecatalog-viewer-about/c-html5-20-ecatalog-viewer-localization.md#concept-cbfc39344c494eb7b9f6a272cff0cc74)。

範例 — 若要設定具有64 x 34 「取消」按鈕和82 x 34 「傳送電子郵件」按鈕的對話方塊頁尾。 每個按鈕狀態的文字顏色和背景顏色都不同：
...
範例 — 將表單設定為有一個畫素灰色邊框，在所有輸入欄位周圍有九個畫素的內距。 對於驗證失敗的欄位，使用相同的紅色邊框。 要有250畫素寬的終止欄位。 最後，其餘的輸入欄位寬度為300畫素：
...
當使用者輸入不正確的輸入資料，並且內嵌驗證失敗時。 或者，當在提交表單時對話方塊必須呈現錯誤或確認訊息時，對話方塊內文的頂端會顯示訊息。 若要控制CSS類別，請使用下列CSS類別選取器：
...
範例 — 若要設定訊息使用十點粗體字型，請有25畫素的行高、20畫素的左側邊框間距，並使用驚歎號圖示。 最後，如果發生錯誤，則顯示為紅色文字，如果成功，則顯示為無圖示和綠色文字：
