...
>[!NOTE]
>
>此訊息支援 `state` 屬性選取器，其可能值如下： `verifyerror`， `senderror`、和 `sendsuccess`. 值 `verifyerror` 在因內嵌輸入驗證失敗而顯示訊息時設定。 值 `senderror` 在後端電子郵件服務回報錯誤時設定。 值  `sendsuccess` 是成功傳送電子郵件時設定。 如此一來，即可根據對話方塊狀態，以不同方式設定訊息樣式。
