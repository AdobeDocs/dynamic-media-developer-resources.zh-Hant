...
建立自訂CSS時，請記得檢視器會將`.s7ecatalogviewer`類別指派給其容器DOM元素。 如果使用與`style=`命令一起傳遞的外部CSS檔案，請在CSS規則的子體選擇器中使用`.s7ecatalogviewer`類作為父類。 如果您要在網頁上執行內嵌樣式，請額外以容器DOM元素的ID限定此選取器，如下所示：
