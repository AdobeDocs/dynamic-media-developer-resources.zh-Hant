---
title: 自定義eCatalog查看器
description: eCatalog Viewer的所有可視化自定義和大多數行為自定義都通過建立自定義CSS來完成。
keywords: 響應
...
# 自定義eCatalog查看器{#customizing-ecatalog-viewer}

eCatalog Viewer的所有可視化自定義和大多數行為自定義都通過建立自定義CSS來完成。
...
建立自定義CSS時，切記查看器指定 `.s7ecatalogviewer` 類到其容器DOM元素。 如果使用與 `style=` 命令，使用 `.s7ecatalogviewer` 類作為CSS規則的後代選擇器中的父類。 如果要在網頁上執行嵌入樣式，還應使用容器DOM元素的ID來限定此選擇器，如下所示：
...
為幫助建立響應性設計的CSS，查看器支援CSS標籤。 特殊CSS類根據運行時查看器大小和當前設備上使用的輸入類型動態分配給頂級查看器容器元素。
...
使用CSS標籤是構建響應性設計的CSS的最靈活方法。 它不僅允許您針對設備螢幕大小，還允許您針對實際的查看器大小，這對於響應性設計的頁面佈局可能非常有用。
...
以下是適用於eCatalog Viewer的用戶介面元素引用文檔：
