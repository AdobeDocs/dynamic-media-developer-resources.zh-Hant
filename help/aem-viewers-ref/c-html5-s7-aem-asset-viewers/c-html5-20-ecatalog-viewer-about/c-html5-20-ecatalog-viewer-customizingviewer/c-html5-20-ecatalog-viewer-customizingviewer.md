---
title: 自訂eCatalog檢視器
description: eCatalog檢視器的所有視覺化自訂和大多數行為自訂，都是透過建立自訂CSS來完成。
keywords: 回應式
...
eCatalog檢視器的所有視覺化自訂和大多數行為自訂，都是透過建立自訂CSS來完成。
...
建立自訂CSS時，請記得檢視器指派 `.s7ecatalogviewer` 類別至其容器DOM元素。 如果您使用的是外部CSS檔案，連同 `style=` 命令，使用 `.s7ecatalogviewer` 類別作為CSS規則的子體選取器中的上層類別。 如果您要在網頁上執行內嵌樣式，也請以容器DOM元素的ID來限定此選取器，如下所示：
...
為協助您建立回應式設計的CSS，檢視器支援CSS標籤。 系統會根據執行階段檢視器大小和目前裝置上使用的輸入類型，以動態方式為頂層檢視器容器元素指派特殊的CSS類別。
...
使用CSS標籤是建置回應式設計之CSS最有彈性的方式。 它不僅可讓您鎖定裝置螢幕大小，還可鎖定實際的檢視器大小，這對回應式設計的頁面配置可能會很實用。
