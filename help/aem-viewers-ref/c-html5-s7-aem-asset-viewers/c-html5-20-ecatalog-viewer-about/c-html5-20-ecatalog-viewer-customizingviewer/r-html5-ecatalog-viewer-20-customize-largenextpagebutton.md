---
title: 「大」下一頁按鈕
description: 按一下或點擊此按鈕，用戶將進入目錄的下一頁。 此按鈕顯示在主控制欄中。 此按鈕不顯示在手機上以保存螢幕不動產。 可以使用CSS調整此按鈕的大小、外觀和位置。
...
# 「大」下一頁按鈕{#large-next-page-button}

選擇或點擊此按鈕，用戶將進入目錄的下一頁。 此按鈕顯示在主控制欄中。 此按鈕不顯示在手機上以保存螢幕不動產。 可以使用CSS調整此按鈕的大小、外觀和位置。
...
示例 — 設定一個大型的下一頁按鈕，該按鈕為56 x 56像素，垂直居中並定位在右側查看器邊框上，並為四個不同按鈕狀態中的每個狀態顯示不同的影像。
