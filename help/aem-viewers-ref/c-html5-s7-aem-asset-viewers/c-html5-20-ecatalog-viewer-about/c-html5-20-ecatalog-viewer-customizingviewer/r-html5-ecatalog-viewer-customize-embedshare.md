...
範例 — 若要使用邊框間距、24 x 14像素圖示和粗體16點標題來設定對話方塊標題。 28 x 28像素的「關閉」按鈕，從頂部放置兩個像素，從對話框容器的右側放置兩個像素：
...
範例 — 若要在顯示預先定義的項目時將內嵌大小下拉式方塊設定為300像素寬，而在顯示自訂大小時設定為110像素寬：
...
範例 — 若要設定寬28像素的捲軸，並且從捲軸面板的上、右和下方有8像素邊距：
...
示例 — 要設定28 x 32像素的捲動按鈕，並且每個狀態的圖稿都不同：
