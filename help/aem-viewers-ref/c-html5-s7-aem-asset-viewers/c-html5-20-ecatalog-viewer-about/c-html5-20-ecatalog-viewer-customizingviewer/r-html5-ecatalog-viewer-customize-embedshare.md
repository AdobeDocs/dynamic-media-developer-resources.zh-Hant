...
示例 — 要設定對話框標題，其中包含填充、24 x 14像素表徵圖和16點粗體標題。 還有一個28 x 28像素的「關閉」按鈕，從頂部放置兩個像素，從對話框容器右側放置兩個像素：
...
示例 — 要設定一個對話框頁腳，其中包含64 x 34取消按鈕、82 x 34全選按鈕，並且每個按鈕狀態的文本顏色和背景顏色不同：
...
示例 — 要將主對話框區域設定為300像素高度，具有10像素邊距，並使用白色背景：
...
示例 — 要將文本副本設定為430像素寬，並在底部具有10像素填充：
...
嵌入大小標籤和下拉清單位於對話框底部，並放入由以下CSS類選擇器控制的容器：
...
示例 — 要設定寬度為28像素且從滾動面板的頂部、右側和底部有八像素邊距的捲動條：
...
示例 — 要設定一個28 x 45像素的捲動條拇指，其頂部和底部有10個像素邊距，並且每種狀態具有不同的圖稿：
...
示例 — 要設定28 x 32像素且每種狀態具有不同圖稿的滾動按鈕：
