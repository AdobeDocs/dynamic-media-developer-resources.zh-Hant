---
title: 最後一頁按鈕
description: 按一下或點選此按鈕會將使用者導向到目錄中的最後一頁。 此按鈕會顯示在桌上型電腦系統和平板電腦的主要控制列中；在行動電話上，此按鈕會新增至次要控制列。 您可以使用CSS調整此按鈕的大小、外觀和位置。
...
選取或點選此按鈕會將使用者導向到目錄中的最後一頁。 此按鈕會顯示在桌上型電腦系統和平板電腦的主要控制列中；在行動電話上，此按鈕會新增至次要控制列。 您可以使用CSS調整此按鈕的大小、外觀和位置。
...
範例 — 設定最後一個頁面按鈕，其位置為28 x 28畫素，距底部為4畫素，距主控制列的左邊緣為220畫素。 最後，針對四種不同的按鈕狀態分別顯示不同的影像。
