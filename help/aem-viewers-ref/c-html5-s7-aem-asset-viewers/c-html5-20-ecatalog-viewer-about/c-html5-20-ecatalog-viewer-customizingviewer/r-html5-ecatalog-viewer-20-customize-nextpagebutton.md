...
選取或點選此按鈕，即可讓使用者進入目錄的下一頁。 此按鈕出現在主控制欄中。 行動電話上不會顯示此按鈕以儲存螢幕空間。 您可以使用CSS來調整此按鈕的大小、外觀和位置。
...
範例 — 設定下一頁按鈕，該按鈕為28 x 28像素，從底部放置4像素，從主控制條的右邊放置250像素。 最後，顯示不同的影像，來呈現四種不同的按鈕狀態。
