...
>[!NOTE]
>
>此按鈕同時支援 `state` 和屬 `selected` 性選擇器，可用來將不同的外觀套用至不同的按鈕狀態。 特別地， `selected='true'` 對應於「靜音」狀態， `selected='false'` 並對應於「靜音」狀態。
...
在上述代碼示例中，卷級別設定為 `0` 在元件 `mutableVolume` 上。 然後，相同的元件會停用，因此使用者無法使用。
