...
# 自定義視頻查看器{#customizing-video-viewer}
...
建立自定義CSS時，請記住查看器會指定 `.s7videoviewer` 類到其容器DOM元素。 如果使用與 `style=` 命令，使用 `.s7videoviewer` 類作為CSS規則的後代選擇器中的父類。 如果在網頁上嵌入樣式，還應按如下方式使用容器DOM元素的ID限定此選擇器：
...
為幫助您建立響應性設計的CSS，查看器支援動態分配給頂級查看器容器元素的特殊CSS類的CSS標籤。 該分配基於運行時查看器大小和當前設備上使用的輸入類型。
...
下面的示例CSS將帶滑鼠輸入的系統上的播放/暫停按鈕大小設定為28 x 28像素，觸摸設備上的56 x 56像素。 此外，如果查看器大小太小，它會完全隱藏按鈕：
...
* CSS內到外部資產的所有路徑都根據CSS位置而不是查看器HTML頁的位置進行解析。 將預設CSS複製到其他位置時，請記住為此規則帳戶。 複製自定義CSS中的預設資產或更新路徑。
* 點陣圖圖稿的首選格式為PNG。
