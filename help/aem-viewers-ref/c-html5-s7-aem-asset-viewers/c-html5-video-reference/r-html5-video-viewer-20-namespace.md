...
不過，有些進階使用案例會要求網頁參考使用中的內部SDK元件 `getComponent()` 檢視器API，接著再使用SDK本身API的所有彈性。
...
無論是哪種情況，檢視器內SDK使用的名稱空間都會 `s7viewers` 或 `s7classic` 做為前置詞。 而且，它和一般情況不同 `s7sdk` SDK使用手冊或SDK API檔案中使用的名稱空間。 因此，撰寫可與內部檢視器元件通訊的自訂應用程式程式碼時，必須使用完整限定的SDK名稱空間。
