---
title: 視訊
description: 視頻查看器是播放以H.264格式編碼的流式和漸進式視頻的視頻播放器。 它是從Dynamic Media Classic或Adobe Experience Manager和Dynamic Media運送的。
keywords: 響應
...
視頻查看器是播放以H.264格式編碼的流式和漸進式視頻的視頻播放器。 它是從Dynamic Media Classic或與Dynamic MediaExperience Manager運送的。
...
當基礎系統支援時，Video Viewer將HLS格式的HTML5流視頻回放使用其預設配置。 在不支援HTML5流式傳輸的系統上，查看器將返回HTML5逐步視頻傳輸。
...
## 使用視頻查看器 {#section-f21ac23d3f6449ad9765588d69584772}

視頻查看器表示一個主JavaScript檔案和一組幫助檔案 — 單個JavaScript包含該查看器在運行時下載的所有Viewer SDK元件、資產和CSS。

可以使用隨IS-Vievers提供的生產就緒HTML頁，在彈出模式下使用視頻查看器。 或者，您可以在嵌入式模式下使用查看器，在該模式下，使用文檔化的API將查看器整合到目標網頁中。
...
## 與視頻查看器交互 {#section-ab66eb6955aa4a8aa6d14a3b3acfed3f}

視頻查看器提供一組用於視頻播放的標準用戶介面控制項，如播放/暫停按鈕、視頻掃描器視頻時間氣泡、播放時間/總時間指示器、音量控制、全屏按鈕和隱藏字幕切換。 所有這些控制項都分組到查看器用戶介面底部的控制欄中。
...
## 使用視頻查看器的社交媒體共用工具 {#section-907d316fe1da4b87abb9775f02464704}

視頻查看器支援社交媒體共用工具。 它們可作為用戶介面中的單個按鈕使用，當用戶按一下或輕擊該按鈕時，該按鈕會擴展到共用工具欄。
...
建議您使用現成HTML頁面進行彈出操作模式。 它叫 [!DNL VideoViewer.html] 它位於 [!DNL html5/] 標準IS查看器部署的子資料夾：
...
1. 將查看器JavaScript檔案添加到網頁。
   ...
   建立查看器要求在HTML頭中添加指令碼標籤。 在使用查看器API之前，請確保包括 [!DNL FlyoutViewer.js]。 的 [!DNL FlyoutViewer.js] 檔案位於 [!DNL html5/js/] 標準IS查看器部署的子資料夾：
...
1. 設定查看器大小
   ...
   請參閱 [自定義視頻查看器](../../c-html5-s7-aem-asset-viewers/c-html5-video-reference/c-html5-video-viewer-20-customizingviewer/c-html5-video-viewer-20-customizingviewer.md#concept-072a52b10b5f4c0789393dc6e2134c0e) 的子菜單。
...
1. 建立和初始化查看器。
   ...
   完成上述步驟後，將建立 `s7viewers.VideoViewer` 類，將所有配置資訊傳遞給其建構子，並調用 `init()` 的子常式。 配置資訊作為JSON對象傳遞給建構子。 至少，此對象應 `containerId` 包含查看器容器ID和嵌套名稱的欄位 `params` 具有查看器支援的配置參數的JSON對象。 在這個例子中， `params` 對象必須至少將Image Serving URL傳遞為 `serverUrl` 屬性，視頻伺服器URL傳遞為 `videoserverurl` 及初始資產 `asset` 的下界。 基於JSON的初始化API允許您使用單行代碼建立和啟動查看器。
   ...
   以下是建立查看器實例、將最小必要配置選項傳遞給建構子並調用 `init()` 的雙曲餘切值。 此示例假定 `videoViewer` 是查看器實例， `s7viewer` 是佔位符的名稱 `DIV`。 [!DNL http://s7d1.scene7.com/is/image/] 是影像服務URL, [!DNL http://s7d1.scene7.com/is/content/] 是視頻伺服器URL, [!DNL Scene7SharedAssets/Glacier_Climber_MP4] 是資產。
