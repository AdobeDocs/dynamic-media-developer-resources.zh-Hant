---
description: 視訊檢視器是視訊播放器，會播放以H.264格式編碼的串流和漸進式視訊。 這是由Dynamic Media Classic或Adobe Experience Manager搭配Dynamic Media提供。
keywords: 回應式
...
視訊檢視器是視訊播放器，會播放以H.264格式編碼的串流和漸進式視訊。 這是從Dynamic Media Classic提供，或透過Dynamic MediaExperience Manager提供。
...
回應式設計內嵌假設檢視器必須在執行階段中調整大小，以回應其容器`DIV`的大小變更。 最常見的使用案例是將檢視器新增至使用彈性頁面版面的網頁。

在回應式設計內嵌模式中，檢視器的運作方式會因網頁大小其容器`DIV`而有所不同。 如果網頁僅設定容器的寬度`DIV`，保持高度不受限制，則檢視器會根據所使用資產的外觀比例自動選擇其高度。 此方法可確保資產完全符合檢視，而無需在側邊填補。 此使用案例最常用於使用回應式設計配置架構(例如Bootstrap或Foundation)的網頁。
...
1. 設定檢視器大小
   ...
   您可以直接將大小調整放在CSS上的HTML頁面，或放在自訂檢視器CSS檔案中。 它稍後會指派給Dynamic Media Classic中的檢視器預設集記錄，或使用style命令明確傳遞。
   ...
   您可以在Dynamic Media Classic的檢視器預設集記錄中設定`stagesize`修飾元，或以包含`params`集合的檢視器初始化程式碼明確傳遞它。 或以API呼叫的形式（如命令參考區段所述），如下列所示：
...
1. 建立和初始化檢視器。
   ...
   同時，容器元素還不一定是網頁版面的一部分。 例如，可使用指派給它的`display:none`樣式來隱藏它。 在此情況下，檢視器會延遲其初始化程式，直到網頁將容器元素帶回版面的那一刻為止。 發生此動作時，檢視器載入會自動恢復。
...
如果定義了寬度和高度的回應式設計內嵌，則網頁樣式會有所不同；它會提供「holder」 `DIV`的兩種大小，並將其置於瀏覽器視窗中。 此外，網頁還將`HTML`和`BODY`元素的大小設定為100%:
