---
title: 設定指示器
description: 設定指示器是當在觸摸設備上使用查看器時在色板上呈現的一系列點。 當滾動按鈕不可用時，這些點可幫助用戶瀏覽縮略圖頁面。
...
設定指示器是當在觸摸設備上使用查看器時在色板上呈現的一系列點。 當滾動按鈕不可用時，這些點可幫助用戶瀏覽縮略圖頁面。
...
示例 — 要建立一個設定為15 x 15像素的設定指示器點，其中包含2個像素水準邊距、5個像素頂邊距、1個像素底邊距、12像素半徑、#D5D3D3預設顏色和#939393用顏色：
