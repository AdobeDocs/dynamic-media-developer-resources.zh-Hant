---
description: 色票由一列縮圖影像組成，左側和右側有可選的捲動按鈕。 只有當所有縮圖無法符合容器寬度時，捲動按鈕才會顯示在案頭上。 在行動裝置上，或如果縮圖可符合容器寬度，則不會顯示捲動按鈕。
...
色票由一列縮圖影像組成，左側和右側有可選的捲動按鈕。 只有當所有縮圖無法符合容器寬度時，捲動按鈕才會顯示在案頭上。 在行動裝置上，或如果縮圖可符合容器寬度，則不會顯示捲動按鈕。
...
<table id="table_565B354FEA814804A0BE3978E1242110"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p> 每個縮圖周圍的水準和垂直邊界的大小。 實際縮圖間距等於為<span class="codeph"> .s7thumbcell </span>設定的左邊距和右邊距之和。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
>[!NOTE]
>
>此按鈕支援`state`屬性選擇器，可用於將不同的外觀應用於不同的按鈕狀態：`up`、`down`、`over`和`disabled`。

按鈕工具提示可翻譯。 請參閱[使用者介面元素的本地化](../../../c-html5-s7-aem-asset-viewers/c-html5-20-zoom-viewer-about/c-html5-20-zoom-viewer-localization.md#concept-cbfc39344c494eb7b9f6a272cff0cc74)。
