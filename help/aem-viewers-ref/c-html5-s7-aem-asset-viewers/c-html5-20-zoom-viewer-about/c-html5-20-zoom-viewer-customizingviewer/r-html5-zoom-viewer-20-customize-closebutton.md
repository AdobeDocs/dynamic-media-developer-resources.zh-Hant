---
title: 關閉按鈕
description: 選擇或點選此按鈕將關閉容納網頁。 此按鈕僅在關閉按鈕參數設定為1時顯示。 您可以使用CSS來調整此按鈕的大小、外觀和位置。
...
選擇或點選此按鈕將關閉容納網頁。 此按鈕僅在關閉按鈕參數設定為1時顯示。 您可以使用CSS來調整此按鈕的大小、外觀和位置。
...
範例 — 設定一個32 x 32像素的「關閉」按鈕，並從檢視器的右上角放置6個像素。 最後，針對四個不同的按鈕狀態顯示不同的影像。
