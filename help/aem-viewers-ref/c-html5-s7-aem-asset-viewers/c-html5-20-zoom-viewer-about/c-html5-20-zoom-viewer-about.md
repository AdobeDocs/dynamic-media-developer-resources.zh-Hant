...
您可以使用隨IS檢視器提供的生產就緒型HTML頁面，或在內嵌模式中使用縮放檢視器，透過記錄的API將其整合至目標網頁。
...
不同的網頁對檢視者行為有不同的需求。 有時，網頁會提供連結，當選取此連結時，即會在個別瀏覽器視窗中開啟檢視器。 在其他情況下，則需直接將檢視器內嵌在托管頁面中。 在後一種情況下，網頁可能具有靜態版面，或使用在不同裝置上或針對不同瀏覽器視窗大小顯示不同的回應式設計。 為了滿足這些需求，檢視器支援三種主要操作模式：快顯視窗、固定大小內嵌和回應式設計內嵌。
...
建議您對快顯操作模式使用現成可用的HTML頁面。 系統會呼叫現成可用的HTML頁面 `ZoomViewer.html` 位於 `html5/` 標準IS-Viewers部署的子資料夾，如下所示：
...
1. 將檢視器JavaScript檔案新增至您的網頁。
   ...
   建立檢視器需要在HTML標題中新增指令碼標籤。 使用檢視器API之前，請務必加入 [!DNL ZoomViewer.js]. 此 [!DNL ZoomViewer.js] 檔案位於 [!DNL html5/js/] 標準IS — 檢視器部署的子資料夾：
...
1. 設定檢視器大小。
   ...
   要使主視圖尺寸保持靜態，請為內部以絕對單位定義查看器尺寸 `Container` SDK元件使用 `.s7zoomviewer` `.s7container` CSS選取器，或使用 `stagesize` 修飾詞。
...
1. 建立和初始化檢視器。
   ...
   完成上述步驟後，請建立例項 `s7viewers.ZoomViewer` 類，將所有配置資訊傳遞到其建構子並調用 `init()` 方法。

   設定資訊會以JSON物件的形式傳遞至建構函式。 至少，此物件應具有 `containerId` 欄位，其中包含檢視器容器ID的名稱和巢狀 `params` JSON物件，以及檢視器支援的設定參數。 在此情況下， `params` 物件至少必須以 `serverUrl` 屬性及初始資產 `asset` 參數。 JSON型初始化API可讓您使用一行程式碼來建立和啟動檢視器。
