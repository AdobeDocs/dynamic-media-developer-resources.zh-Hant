...
# 自定義內聯縮放查看器{#customizing-inline-zoom-viewer}
...
此外，在內聯縮放功能需要以下CSS聲明時，還必須從預設查看器CSS中保留它：
...
建立自定義CSS時，請記住查看器會指定 `.s7flyoutviewer` 類到其容器DOM元素。 如果使用與 `style=` 命令，使用 `.s7flyoutviewer` 類作為CSS規則的後代選擇器中的父類。 如果要在網頁上執行嵌入樣式，還應使用容器DOM元素的ID來限定此選擇器，如下所示：
...
為幫助建立響應性設計的CSS，查看器支援動態分配給頂級查看器容器元素的特殊CSS類的CSS標籤。 這些分配基於運行時查看器大小和當前設備上使用的輸入類型。
...
使用CSS標籤是構建響應性設計的CSS的最靈活方法。 它不僅允許目標設備螢幕大小，還允許實際查看器大小，這對於響應性設計的網頁佈局可能非常有用。
...
## CSS繁體 {#section-b0af39db1af74561aea9fddcc8cdc2c7}

許多查看器用戶介面元素使用點陣圖圖稿設定樣式，並具有多個不同的可視狀態。 一個很好的例子是按鈕，它通常至少有三種不同的狀態：&quot;up&quot;、&quot;over&quot;和&quot;down&quot;。 每個狀態都需要分配其自己的點陣圖圖稿。

使用經典的造型方法，CSS會針對用戶介面元素的每個狀態對伺服器上的單個影像檔案有單獨的引用。 以下是用於設定滾動按鈕樣式的示例CSS:
