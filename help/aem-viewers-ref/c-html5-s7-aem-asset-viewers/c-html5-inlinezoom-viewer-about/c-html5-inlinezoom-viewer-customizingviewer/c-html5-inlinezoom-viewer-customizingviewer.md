...
當您建立自訂CSS時，請記住檢視器會將類別 `.s7flyoutviewer` 指派給其容器DOM元素。 如果您使用隨命令傳遞的外部CSS檔案， `style=` 請在CSS規 `.s7flyoutviewer` 則的子體選擇器中，使用類別做為上層類別。 如果您要在網頁上執行內嵌樣式，請另外以容器DOM元素的ID來限定此選取器，如下所示：
