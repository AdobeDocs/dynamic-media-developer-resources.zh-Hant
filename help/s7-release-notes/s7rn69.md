...
* **增強的回應式支援** -針對每個斷點定義不同的影像預設集或修飾元，進一步最佳化回應式影像體驗的品質。 請參 [閱使用自適應靜態影像庫](https://docs.adobe.com/content/help/en/dynamic-media-developer-resources/image-serving-api/image-serving-api/responsive-static-image-library/c-about-responsive-static-image-library.html)。
...
* 透過編碼改善彈性調整彈性和效能。
* 現在，現成可用的視訊字幕與檢視器 `Universal_HTML5_MixedMedia_dark` 都 `Universal_HTML5_MixedMedia_light` 有。 請參閱[增加註解至視訊](https://docs.adobe.com/content/help/en/dynamic-media-classic/using/video/adding-captions-video.html)。
