...
description: 「說明Adobe Scene7 Publishing System 6.9 (Adobe Experience Cloud中Adobe Experience Manager解決方案的一部分)最新功能、增強功能和修正的最新發行說明。」
...
最新發行說明說明說明了Adobe Experience Cloud中Adobe Scene7解決方案一部分的Adobe Experience Manager Publishing System 6.9的最新功能、增強功能和修正。

## Scene7 Publishing System 6.9發行說明（2014年10月） {#topic-1d3011ecca71488abe0d8156a9fc1b97}

說明最新功能、增強功能和修正的最新發行說明 [!DNL Adobe Scene7 Publishing System] 中的Adobe Experience Manager解決方案的6.9部分 [!DNL Adobe Experience Cloud].

* [Scene7 Publishing System 6.9發行重點說明](s7rn69.md#section-2ed72b7902be4509876d81f25b5632ce)
...
* [Scene7 Publishing System 6.9錯誤修正](s7rn69.md#section-86dc0e8eb85241dbb449221becb12888)
* [不再支援的功能](s7rn69.md#section-b061ad3d170a45ee8e694f997fd5d8ef)

## Scene7 Publishing System 6.9發行重點說明 {#section-2ed72b7902be4509876d81f25b5632ce}
...
* **檢視器自訂**  — 您現在可以動態地即時設定及修改檢視器樣式和行為。
* **Visual Configurator的安全預覽**  — 在將個人化體驗推送至生產環境之前，先安全地預覽和測試個人化體驗的視覺呈現。
...
* **增強回應式支援**  — 透過為每個中斷點定義不同的影像預設集或修飾元，進一步最佳化回應式影像體驗的品質。 另請參閱 [使用回應式靜態影像程式庫](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/responsive-static-image-library/c-about-responsive-static-image-library.html).
...
* 轉譯器伺服器要求的安全測試 — 核心影像伺服資產和視訊資產的安全測試現已擴充為也包含暈映。 另請參閱 [在公開資產之前先測試資產](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish).
* 監視失效CDN快取功能。 選取CDN失效範本後(請參閱 [一般設定](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/setup/application-setup.html#general-settings))和 **[!UICONTROL 使CDN失效]** 提出要求後，現在會彈出一個指標，估計清除快取所需的時間。
...
* 新增的支援 `setCSS()`， `setModifier()`、和 `dispose()` 方法。
* 新增的支援 `getWidth()`、和 `getHeight()` 方法。
* 新增的支援 `setPortrait()` 和 `setLandscape ()` 方法。
...
* Internet Explorer 9和Internet Explorer 10： eCatalog檢視器在使用JavaScript範本啟動影像地圖後中斷。
* HTML5 MixedMedia檢視器：切換資產型別時，縮放按鈕狀態未重設。
* HTML5 MixedMedia檢視器：如果第一個資產是2dspinset，檢視器會在回應模式中摺疊。
...
## Scene7 Publishing System 6.9錯誤修正 {#section-86dc0e8eb85241dbb449221becb12888}
...
* Scene7 Publishing System無法正確顯示某些規則運算式代碼來重複。
* 如果未選取任何預設集，上傳時會套用所有批次集預設集。

## 不再支援的功能 {#section-b061ad3d170a45ee8e694f997fd5d8ef}

* 此版本已棄用Scene7Creative Suite擴充功能，因為Creative Cloud應用程式不再支援Flash面板。 內容擷取工作流程的進一步發展著重於Adobe Experience Manager Dynamic Media和Creative Cloud。 若為Scene7，仍可使用Scene7 Publishing System中的上傳和匯出功能。
