...
* 垃圾桶中涉及清單檢視的錯誤修正。
* 基於安全性考量，已從「使用者管理」中移除&#x200B;**建立Dynamic Media Classic (Scene7)管理員**&#x200B;使用者功能。
...
* 目錄內容 — `initialframe`並未影響HTML5 eCatalog檢視器行動裝置上的直向模式。
...
* 觸控裝置
   ...
   * 自訂游標在觸控裝置上無法運作。
   * 目前不支援觸控裝置上的頁面縮放功能。 內嵌HTML5檢視器需要具備適當設定的檢視區meta標籤。
