...
最新發行說明說明說明了Adobe Experience Cloud中Adobe Experience Manager解決方案的Adobe Scene7 Publishing System 6.8部分的最新功能、增強功能和修正。

## Scene7 Publishing System 6.8發行說明（2014年5月） {#topic-4517aab837ea4d6d8204425958860e5a}

說明[!DNL Adobe Scene7 Publishing System]中Adobe Experience Manager解決方案[!DNL Adobe Experience Cloud] 6.8部分的最新功能、增強功能和修正的最新發行說明。
...
* 為回應式網頁設計新增支援 — 根據中斷點提供回應式影像服務和回應式HTML5檢視器（影像集、視訊、混合媒體、eCatalog、迴轉集、色票集、縮放），可依據頁面版面配置動態調整大小。
...
* HTML5檢視器可支援回應式設計。
* HTML5檢視器可使用CSS輸入和大小標籤來控制桌上型電腦和行動裝置上的顯示。
...
* HTML5檢視器的`initComplete`處理常式，用於在檢視器之後觸發函式。
* HTML5檢視器的Analytics處理常式，用於支援第三方追蹤。
...
另請參閱[檢視者參考指南](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/homeviewers.html?lang=en)。
...
* 若要防止加入封鎖清單，請將忘記密碼電子郵件從HTML轉換為純文字。
* 已消除預設角色。
* 除了可透過伺服器目錄上傳(ftp)和上傳URL將內容上傳至Scene7 Publishing System之外，您現在也可以使用HTTP POST上傳內容。 如需使用HTTP POST的詳細資訊，請參閱[影像生產系統API](/help/aem-ips-api/c-overview.md)。
...
* autoResize不應位於HTML彈出式檢視器副本URL/內嵌程式碼中。
* HTML5 Video Viewer Copy URL中沒有videoserverurl修飾元。
* 引數`contenturl`來自發佈伺服器HTML5檢視器預覽URL。

**影像**

* 無法在影像發佈設定中將調整後的檢視新增為預設影像。
...
* 初始化公司設定時，Scene7 Publishing System會在API `<defaultValue>`中傳送重複的選項`<createMetadataField>`。

* Flash外掛程式和Scene7 Publishing System因許多資料夾的Flash記憶體不足而當機。
...
* 啟用SSL時無法匯出資產。
* 忘記密碼電子郵件中顯示的密碼不正確。
* 系統會將任何Scene7 Publishing System Web管理員要求重新導向至HTTP。
* 如果未選取預設集，則會在上傳期間套用所有批次集預設集。
* 上傳s7info內容時，影像產生系統會透過HTTP要求s7info。
* 「忘記密碼」電子郵件中的URL是HTTP，即使它來自SSL Scene7 Publishing System。
* HTML格式的「忘記密碼」電子郵件會對特殊字元進行編碼，導緻密碼無法正確顯示。
...
* Adobe Illustrator和Adobe InDesign在2014年5月更新後，預期將與建立Scene7 FXG的外掛程式不相容。 這些不相容性會影響Scene7網頁對列印解決方案的使用者。 如果您使用Scene7網頁列印，請勿將Illustrator和InDesign升級至此Creative Cloud更新。 請改用2014年5月前的版本。 Adobe Inc預計在2014年秋季時間範圍內將Scene7 FXG的建立與較新版本的Illustrator和InDesign恢復相容性。
...
* 此發行版本已棄用動態(Flash)橫幅。 隨著建立、管理和提供個人化網站橫幅的其他技術的出現，此功能採用率一直很低。 Scene7動態(Flash)橫幅功能在伺服器端繼續運作，但已不再受支援。
* 提醒：自上一個Scene7版本起，中繼資料伺服器及關聯API的使用已正式停止支援。
* Scene7 Creative Suite Extensions （Flash面板）已於2014年底淘汰。 內容擷取工作流程的進一步發展著重於Adobe Experience Manager Dynamic Media和Creative Cloud。 如果是Scene7，仍可使用Scene7 Publishing System中的上傳和匯出功能。
...
* 視訊詳細資料檢視的RTMP串流。
* 「電子郵件通訊」和來自Scene7 Publishing System的SendMailSimple.jsp servlet。
...
* 已修正：內嵌在`selectSwatch`函式中時未觸發`initViewer()`方法。
...
* 修正：使用Flash播放時，HTTPS通訊協定會在IE8上產生安全性警告。
* 修正：在使用Flash播放的Safari 5上，以全熒幕模式運作的視訊大小不正確。
* 修正：視訊在iPhone上播放後，無法選取媒體集中的色票。

**縮放檢視**

* 修正：在™上縮放至影像或旋轉裝置後顯示的成品。
