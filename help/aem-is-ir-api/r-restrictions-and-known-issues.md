...
* `\sb` 適用於MS Word和 `text=`Adobe InDesign和Photoshop不要這麼做。

* `\sa` 適用於MS Word和 `text=`Adobe InDesign和Photoshop不要這麼做。
...
* Adobe PhotoshopCS3在預設情況下不保存複合映像的分層PSD檔案。

   *症狀*:

   Adobe PhotoshopCS3分層PSD檔案顯示為黑色，文本中寫有「此分層Photoshop檔案未用複合影像保存」。 在IPS中。

   *解決辦法*︰

   保存Adobe PhotoshopCS3檔案，同時最大程度地啟用相容性。
