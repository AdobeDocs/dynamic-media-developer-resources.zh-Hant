...
* 行數不超過 `\copyfitmaxlines` 設定文字輸入中的明確行數。
* 影像集中必須搭配大括弧和括弧。 如果大括弧和括弧不相符，則必須將URL編碼。
* 伺服器端全域回應時間警示包含錯誤回應。
* 此 `id=` 目前使用時需要命令 `rect=` 含影像或遮色片要求的命令。

## 已知差異textPs=與text= {#section-16ede4c13a7648feb0d2fc93341fd4aa}

* 合成斜體呈現的斜體比使用時少 `text=`.
* 底線比使用時稍微低一點且更薄 `text=`.
* `\expnd` 和 `\expndtw` 搭配高負值使用會導致在使用時，字元彼此排在前 `text=`.

* `\charscaley` 縮放方式與使用時不同 `text=` 但不會影響行高。

* 如果文字的最後一行不符合，則會捨棄整行，而不會顯示為截斷。
* `\slmult` 和 `\sl` 行為與MS Word和 `text=`，它們只會對目前和後續段落生效。

* `\sb` 適用於MS Word和 `text=`、Adobe InDesign和 [!DNL Photoshop] 請勿這麼做。

* `\sa` 適用於MS Word和的最後一個段落 `text=`、Adobe InDesign和 [!DNL Photoshop] 請勿這麼做。

## 回溯相容性 {#section-a76842f751944f4fb664af296d064122}

* 逸出底線字元( `\_`)無法搭配使用的所有字型使用 `textPs=`
...
* 設定時 `errorRedirect.rootUrl` 此屬性中定義的IP位址必須包含在規則集中 `<addressfilter>` 標籤值。
  ...
  *範例*:
  ...
  IP位址為10.10.10.10的伺服器B會設定 `<addressfilter>` 標籤值，以包含其IP位址(10.10.10.10)。

* 具有定位的點文字和文字路徑可能會顯示剪裁。
* `text=` 僅適用 `\sa` 和 `\sb` 到整個文字區塊，而不是每個段落。

* 使用URL中定義的一家公司，以及為定義的另一家公司時 `src=` 或 `mask=` 修飾元，您必須在為定義的公司加上正斜線作為前置詞 `src=` 或 `mask=` 讓此表單請求運作。
...
* 非金字塔Tiff或暈映請求會產生類似的錯誤訊息，以便
  ...
  *&quot;影像 `C:\Program Files\Scene7\ImageRendering\resources\MyVignette.vnt` 沒有有效的DSF，且2.25MPixel的區域超過最大2MPixel」* .

  最佳實務建議使用金字塔Tiff和暈映。 如果您需要使用非金字塔形浮雕或暈映，請依照下列指示增加大小限制。

  *解決方法*：

  對於影像演算非金字塔暈映，請將IrMaxNonPyrVignetteSize的屬性值增加到 [!DNL install_root/ImageServing/bin/ImageServerRegistry.xml] 組態檔。

  對於「影像伺服」非金字塔TIFF，請增加 `MaxNonDsfSize` 在 [!DNL install_root/ImageServing/bin/ImageServerRegistry.xml] 組態檔。
...
* 將ICC設定檔指派給CMYK/JPEG回覆影像，會導致某些瀏覽器中的顏色反轉。*解決方法*：

  使用變更回覆影像格式 `fmt=`

* HTTP回應影像資料在壓縮後（包括檔案標頭）的大小上限為16 MB。
* 「 ..」 不允許在HTTP要求的任何路徑元素中使用。
* 解除安裝可能會從移除使用者建立或修改的檔案 *[!DNL install_root]* 或任何子資料夾。 在解除安裝之前，將此類檔案複製到其他位置。

## 僅適用於影像伺服的限制 {#section-b08ad535e4454265b8157dec244c4faf}

* RTF中的前景顏色( `\cf`)命令不支援PhotoFont文字。
...
* 「影像伺服」不會寫入非關閉 [!DNL Photoshop] 正確路徑。
...
* 使用 `text=` 使用自行調整圖層的字串，不支援使用多個設定進行行對齊的RTF字串。
...
* 裁切目前正在使用 `bgColor=` 而非 `color=` 以填滿任何新展開的區域。
...
`ImageConvert`在以「 」停止時，有時會因分段錯誤而當機 `control-c`.
