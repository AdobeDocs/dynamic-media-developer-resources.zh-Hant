...
另請參閱 [文字格式](../../../../../is-api/http-ref/image-serving-api-ref/c-http-protocol-reference/c-text-formatting/c-text-formatting.md#concept-0d3136db7f6f49668274541cd4b6364c).

`textPs=` 支援延伸功能，例如齊行，將文字排入定義為非矩形的區域 `textFlowPath=` 和/或 `textFlowXPath=`，並沿著定義的任意路徑轉譯文字 `textPath=`.

## 屬性 {#section-a289dc26b6534b41998b1e241d5f2f92}

圖層屬性。 套用至 `layer=0` 如果 `layer=comp`. 互斥 `src=` 和 `text=` 在相同圖層中。 最後一次出現的 `text=`， `textPs=`、和 `src=` 會取勝，並判斷這是影像還是文字圖層。 被效果圖層忽略。
