...
每個影像目錄（和預設目錄）可以定義一組ICC設定檔，這些設定檔構成此目錄的預設色域 — 灰階、RGB和CMYK資料各有一個輸入和一個輸出設定檔。 請參閱
...
| `JapanColorNewspaper` | Japan Color 2002報紙 | JapanColor2002Paper.icc |
...
| `SheetfedCoated` | U.S. Sheetfed Coated v2 | Usheetfedcoated.icc |
