---
description: 應考慮數個伺服器行為問題。
seo-description: 應考慮數個伺服器行為問題。
seo-title: 伺服器行為注意事項
solution: Experience Manager
title: 伺服器行為注意事項
topic: Scene7 Image Serving - Image Rendering API
uuid: cca17322-4058-43cc-b022-ef8df52c0888
translation-type: tm+mt
source-git-commit: 94a26628ec619076f0942e9278165cc591f1c150
workflow-type: tm+mt
source-wordcount: '61'
ht-degree: 0%

---


# 關於伺服器行為的說明{#notes-on-server-behavior}

應考慮數個伺服器行為問題。

* [合成畫布](r-the-compositing-canvas.md)
* [圖層轉換](r-layer-transforms.md)
* [縮圖縮放](r-thumbnail-scaling.md)
* [縮圖規則](r-thumbnail-rules.md)
* [圖層放置](r-layer-placement.md)
* [影像的檢視變形](r-view-transform-for-images.md)
* [縮圖檢視變形](r-view-transform-for-thumbnails.md)
