---
description: 本節說明所有可編輯的組態設定，依感興趣區域分組。
seo-description: 本節說明所有可編輯的組態設定，依感興趣區域分組。
seo-title: 伺服器設定
solution: Experience Manager
title: 伺服器設定
topic: Scene7 Image Serving - Image Rendering API
uuid: dc4deefe-c354-4d01-a1d2-815597994adc
translation-type: tm+mt
source-git-commit: 5717550d2dea8ec086875e770ff8f200aaa75ff3

---


# 伺服器設定{#server-settings}

本節說明所有可編輯的組態設定，依感興趣區域分組。

* [一般](r-general.md)
* [內容資料夾](r-content-data-folders.md)
* [伺服器快取](r-server-caches.md)
* [影像大小限制](r-image-size-limits.md)
* [影像目錄服務](r-image-catalog-service.md)
* [SSL](r-ssl.md)
* [伺服器](r-server-server-settings.md)
* [存取記錄](r-access-logging.md)
* [Debug_trace記錄](r-debug-trace-logging.md)
* [重定向錯誤](r-error-redirection.md)
* [快取叢集](r-cache-clustering.md)
* [監控和警報系統](r-monitoring-and-alerting-system.md)
* [警報閾值](r-alert-thresholds.md)
* [SVG](r-svg.md)
* [媒體集響應](r-media-set-responses.md)
