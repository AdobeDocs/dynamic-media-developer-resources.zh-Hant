...
資料夾 [!DNL Platform Server] 寫入日誌檔案。 這可以是絕對路徑，或相對於 *`install_folder`*. 預設為 [!DNL  *`install_folder`*/logs].
...
寫入訪問日誌資料的檔案的名稱前置詞。 日期和檔案尾碼( [!DNL  *`yyyy`*-*`mm`*-*`dd`*.log])附加至指定的字串。 訪問日誌檔案的名稱必須與跟蹤日誌檔案的名稱不同。 預設為 &quot; `access-`&quot;.

## TC::pattern — 訪問日誌模式 {#section-22775ea85cee444d8a7d7336a3b1feef}

指定 [!DNL Platform Server] 訪問日誌記錄。 模式字串指定用其對應值替換的變數。 模式字串中的所有其他字元都以字面方式傳送到記錄。

若要使用快取熱身公用程式，必須使用空格作為欄位分隔符號。 此 [!DNL Platform Server] 將欄位值中的所有空格和「%」字元替換為 `%20` 和 `%25`，分別為。
...
<table id="table_7A07AFF34B5040A5B30F5735CFE9428F"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> <p> <span class="codeph"> %R </span> </p> </td> 
   <td> <p>與相同 <span class="codeph"> %r </span>，但會將有限的HTTP編碼套用至URI，以避免記錄檔剖析問題。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>[!DNL Platform Server] 快取密鑰（快取檔案資料夾/名稱）。 </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> %{CacheUse}r </span> </p> </td> 
   <td> <p>[!DNL Platform Server] 快取管理關鍵字： <span class="codeph"> {重複使用 |已建立 |已更新 |遠程 | REMOTE_CREATED | REMOTE_UPDATED |遠程快取 |已驗證 |已忽略 |未定義} </span>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>此 <span class="codeph"> etag </span> 回應標題值（回應資料的MD5簽章）。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>快取群集中傳送快取項或「 — 」(如果 <span class="codeph"> CacheUse </span> 都不是 <span class="codeph"> REMOTE_CREATED </span> no <span class="codeph"> REMOTE_UPDATED </span>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>的大寫值 <span class="codeph"> req= </span>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>所需時間 [!DNL Platform Server] 在將資料寫入輸出流後發送響應。 </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> %{大小}r </span> </p> </td> 
   <td> <p>贊 <span class="codeph"> %B </span>，但包含304（未修改）回應的值。 </p> </td> 
  </tr> 
  <tr> 
