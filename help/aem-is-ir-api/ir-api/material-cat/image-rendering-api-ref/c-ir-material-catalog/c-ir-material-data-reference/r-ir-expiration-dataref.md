...
瀏覽器會使用檔案的到期時間來管理快取。 將請求傳遞至伺服器之前，瀏覽器會檢查其快取，檢視檔案是否已下載。 若是如此，且檔案尚未過期，瀏覽器會傳送條件式GET要求（例如使用If-Modified-Since HTTP要求標頭），而非一般的GET要求。 伺服器可以選擇以&#39;304&#39;狀態回應，而不傳輸影像。 然後瀏覽器只需從其快取載入檔案即可。 這可能會大幅提升經常存取資料的整體效能。
