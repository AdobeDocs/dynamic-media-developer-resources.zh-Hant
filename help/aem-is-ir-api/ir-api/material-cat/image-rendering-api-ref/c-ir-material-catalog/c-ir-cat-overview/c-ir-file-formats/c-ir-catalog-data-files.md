...
目錄資料檔案實質上是二維表，它由標頭記錄組成，標識資料列和任意數量的資料記錄（行）。 標題和資料記錄中的欄位皆以單一字元 `<TAB>` 分隔。 記錄由單一( `<CR>` ASCII代碼0xD)、單一( `<LF>` ASCII代碼0xA)或一對 `<CR><LF>` 分隔。

標題記錄必須包含每個資料欄位的確切名稱。 標題列中不允許空白欄位。 資料欄位名稱不區分大小寫。 所有欄位名稱必須是唯一的。

標題和資料記錄中 `<TAB>` 不允許除欄位分隔符以外的空白字元。

在資料記錄中，兩個相鄰的字 `<TAB>` 元代表空白欄位。 空欄位將繼承目錄屬性或伺服器預設值的預設值。

資料欄位不得包 `<CR>`含、 `<LF>`或字 `<TAB>` 元，除非資料值為文字類型，且以單引號或雙引號括住。 資料欄位不得使用HTTP編碼。
