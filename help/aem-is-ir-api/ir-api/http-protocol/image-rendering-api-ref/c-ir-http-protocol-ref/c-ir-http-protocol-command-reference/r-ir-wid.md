---
description: 回覆影像寬度。 指定已呈現影像的縮放比例，使得回覆影像不高於指定值，同時維持影像的長寬比。
...
回覆影像寬度。 指定已呈現影像的縮放比例，使得回覆影像不高於指定值，同時維持影像的長寬比。
...
如果同時指定`wid=`和`hei=`且`wid`/ `hei`與影像的長寬比不同，則不會補充影像。
...
可能發生在要求內的任何位置。 使用`wid=`、`hei=`或`scl=`調整影像大小不會更改響應影像中嵌入的打印解析度值。 如果`scl=`發生在命令序列的`wid=`或`hei=`之後，則忽略。

## 預設 {#section-c7c6efa03d864592a3398b6f1de5a0b0}

如果未指定`wid=`、`hei=`或`scl=`，則回復影像會縮放以符合`attribute::DefaultPix`所定義的大小。 如果`attribute::DefaultPix`為空，則回覆影像的大小將與暈映的檢視影像大小相同。
